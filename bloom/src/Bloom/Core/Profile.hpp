...
#define BLOOM_PROFILE_IMPL(PROFILE, NAME) \
	auto const UTL_UNIQUE_NAME(_profile_var_) = ::bloom::ProfileHandle(PROFILE, NAME)

#define BLOOM_PROFILE(STAGE, ...) \
UTL_VFUNC(BLOOM_PROFILE_, STAGE __VA_OPT__(,) __VA_ARGS__)

#define BLOOM_PROFILE_1(STAGE) BLOOM_PROFILE_IMPL(&::bloom::Profilers::STAGE, __PRETTY_FUNCTION__)
#define BLOOM_PROFILE_2(STAGE, NAME) BLOOM_PROFILE_IMPL(&::bloom::Profilers::STAGE, NAME)
...
namespace bloom {
    ...
    public:
		...
		utl::hashmap<std::string, ProfileResult> const& get() const { return _profiles; }
