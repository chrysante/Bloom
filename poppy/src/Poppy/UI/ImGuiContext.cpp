...
namespace poppy {
	...
	static ImGuiKey toImGuiKeyCode(Key key) {
		switch (key) {
			...
			case Key::delete_: return ImGuiKey_Delete;
	...
	ImFontAtlas* ImGuiContext::sFontAtlas = nullptr;
	
	ImGuiContext::~ImGuiContext() = default;
	
	void ImGuiContext::init(bloom::Application& application, ImGuiContextDescription const& ds) {
...
//		io.ConfigFlags |= ImGuiConfigFlags_ViewportsEnable;
	...
	void ImGuiContext::loadFonts(bloom::HardwareDevice& device, float scaleFactor) {
		if (!sFontAtlas) {
		...
		sFontAtlas->Build();
		{
			...
			poppyAssert(testFont->IsLoaded());
		}
		
		
		auto* const iconFont16 = icons.font(IconSize::_16);
		poppyAssert(iconFont16);
		poppyAssert(iconFont16->IsLoaded());
	
		poppyAssert(icons.font(IconSize::_16)->IsLoaded());
	}
	...
	void ImGuiContext::newFrame(bloom::Window& window) {
		ImGui::SetCurrentContext(context);
		ImGuiIO& io = ImGui::GetIO();
	...
	void ImGuiContext::onInput(bloom::InputEvent e) {
		ImGui::SetCurrentContext(context);
		...
		e.dispatch<InputEventType::mouseMoved>([&](MouseMoveEvent const& event) {
			io.AddMousePosEvent(event.locationInWindow.x,
		...
		e.dispatch<InputEventType::scrollWheel>([&](ScrollEvent const& event) {
			if (event.offset.x != 0.0 || event.offset.y != 0.0) {
				io.AddMouseWheelEvent((float)event.offset.x * 0.1f, (float)event.offset.y * 0.1f);
			}
			return true;
		});
...
//				return true;
	...
	void ImGuiContext::onCharInput(unsigned int code) {
		ImGui::SetCurrentContext(context);
		ImGuiIO& io = ImGui::GetIO();
	...
	void ImGuiContext::doInitPlatform(bloom::HardwareDevice& device, ImGuiContextDescription const& d) {
		
	}
	
	void ImGuiContext::createFontAtlas(bloom::HardwareDevice&) {
	...
	void ImGuiContext::doDrawFramePlatform(bloom::HardwareDevice& device, bloom::Window& window) {
	...
	void ImGuiContext::createFontAtlasPlatform(ImFontAtlas* atlas, bloom::HardwareDevice&) {
