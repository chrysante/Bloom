...
namespace poppy {
	...
	void ViewportCameraController::update(bloom::Timestep time, bloom::Input const& input) {
		using namespace bloom;
		
		data.angleLR  = utl::mod(data.angleLR - input.mouseOffset().x / 180., mtl::constants<>::pi * 2);
	...
	mtl::float3 ViewportCameraController::front() const {
		...
		using std::cos;
		return {
			sin(data.angleUD) * cos(data.angleLR),
