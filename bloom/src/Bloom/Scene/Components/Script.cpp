...
//			node["Type"] =
// ScriptHelpers::SupportedTypesNames[Index]; 			node["Value"] =
// chaiscript::boxed_cast<T>(v); 			return true;
//		}
//		return false;
//	}
//
...
// ScriptHelpers::SupportedTypesNames[Index]) { 			v =
// chaiscript::Boxed_Value(node["Value"].as<T>()); 			return true;
//		}
//		return false;
//	}
//
...
//{
//			(... || tryEncodeAs<ScriptHelpers::TestType<I>, I>(node,
// v));
//		});
//		return node;
//	}
...
//		try {
//			return UTL_WITH_INDEX_SEQUENCE((I,
// ScriptHelpers::SupportedTypesCount), { 				return (... ||
// tryDecodeAs<ScriptHelpers::TestType<I>, I>(node, v));
//			});
//		}
//		catch(std::exception const& e) {
//			BL_DEBUGBREAK("Whats going on here?");
//			return false;
//		}
//	}
// };
//
// YAML::Node
// YAML::convert<bloom::ScriptComponent>::encode(bloom::ScriptComponent const&
// s) { 	Node root; 	root["ClassName"] = s.className; 	if (!s.object) {
// return
// root;
//	}
//	YAML::Node attribs;
...
// bloom::ScriptComponent& s) { 	s.className =
// root["ClassName"].as<std::string>(); 	try { #warning 		throw
// std::runtime_error("Failed to decode script component");
////		s.object =
/// bloom::Application::get().scriptEngine().instanciateObject(s.className); /
/// YAML::Node attribs = root["Attributes"];
////		for (auto&& [name, value]: s.object->get_attrs()) {
////			if (attribs[name].IsDefined()) {
////				auto rhs =
/// attribs[name].as<chaiscript::Boxed_Value>(); /
