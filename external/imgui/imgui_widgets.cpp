...
bool ImGui::CloseButton(ImGuiID id, const ImVec2& pos)
{
    ...
    ImU32 const col = GetColorU32(held ? ImGuiCol_ButtonActive : ImGuiCol_ButtonHovered);
	...
	if (hovered) {
#if POPPY_CUSTOM
		window->DrawList->AddRectFilled(bb.Min + close_button_padding, bb.Max - close_button_padding, col, close_button_rounding);
#else
...
bool ImGui::BeginMenuEx(const char* label, const char* icon, bool enabled, bool* out_pressed)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
...
//    const ImU32 col = GetColorU32((flags & ImGuiTabBarFlags_IsFocused) ? ImGuiCol_TabActive : ImGuiCol_TabUnfocusedActive);
	const ImU32 col = GetColorU32(ImGuiCol_Border);
...
#else
	const ImU32 tab_col = GetColorU32(tab_contents_visible ?
									  (tab_bar_focused ? ImGuiCol_TabActive : ImGuiCol_TabUnfocusedActive) :
									  (tab_bar_focused ? ImGuiCol_Tab       : ImGuiCol_TabUnfocused));
#endif
	
	TabItemBackground(display_draw_list, bb, flags | (docked_window ? ImGuiTabItemFlags_DockSpaceTab : 0), tab_col);
    RenderNavHighlight(bb, id);
...
ImVec2 ImGui::TabItemCalcSize(const char* label, bool has_close_button)
{
    ...
    if (has_close_button)
        size.x += 2 * g.Style.FramePadding.x + (g.Style.ItemInnerSpacing.x + g.FontSize); // We use Y intentionally to fit the close button circle.
    else
...
void ImGui::TabItemBackground(ImDrawList* draw_list, const ImRect& bb, ImGuiTabItemFlags flags, ImU32 col)
{
    ...
    const float rounding = is_dockspace_tab ? 0 : ImMax(0.0f, ImMin((flags & ImGuiTabItemFlags_Button) ? g.Style.FrameRounding : g.Style.TabRounding, width * 0.5f - 1.0f));

    const float y1 = bb.Min.y + (is_dockspace_tab ? 0.0f : 1.0f);
