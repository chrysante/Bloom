...
// Configuration
static int const TABLE_DRAW_CHANNEL_BG0 = 0;
static int const TABLE_DRAW_CHANNEL_BG2_FROZEN = 1;
static int const TABLE_DRAW_CHANNEL_NOCLIP =
    2; // When using ImGuiTableFlags_NoClip (this becomes the last visible
       // channel)
static float const TABLE_BORDER_SIZE =
    1.0f; // FIXME-TABLE: Currently hard-coded because of clipping assumptions
          // with outer borders rendering.
static float const TABLE_RESIZE_SEPARATOR_HALF_THICKNESS =
    4.0f; // Extend outside inner borders.
static float const TABLE_RESIZE_SEPARATOR_FEEDBACK_TIMER =
...
bool ImGui::BeginTable(char const* str_id, int columns_count,
                       ImGuiTableFlags flags, ImVec2 const& outer_size,
                       float inner_width) {
...
bool ImGui::BeginTableEx(char const* name, ImGuiID id, int columns_count,
                         ImGuiTableFlags flags, ImVec2 const& outer_size,
                         float inner_width) {
    ImGuiContext& g = *GImGui;
    ...
    bool const use_child_window =
        (flags & (ImGuiTableFlags_ScrollX | ImGuiTableFlags_ScrollY)) != 0;
    ImVec2 const avail_size = GetContentRegionAvail();
    ...
    int const instance_no = (table->LastFrameActive != g.FrameCount) ?
                                0 :
                                table->InstanceCurrent + 1;
    ImGuiID const instance_id = id + instance_no;
    ImGuiTableFlags const table_last_flags = table->Flags;
    if (instance_no > 0)
        IM_ASSERT(
    ...
    int const table_idx = g.Tables.GetIndex(table);
    ...
    bool const pad_outer_x = (flags & ImGuiTableFlags_NoPadOuterX) ?
                                 false :
                             (flags & ImGuiTableFlags_PadOuterX) ?
                                 true :
                                 (flags & ImGuiTableFlags_BordersOuterV) != 0;
    bool const pad_inner_x = (flags & ImGuiTableFlags_NoPadInnerX) ? false :
                                                                     true;
    float const inner_spacing_for_border =
        (flags & ImGuiTableFlags_BordersInnerV) ? TABLE_BORDER_SIZE : 0.0f;
    float const inner_spacing_explicit =
        (pad_inner_x && (flags & ImGuiTableFlags_BordersInnerV) == 0) ?
            g.Style.CellPadding.x :
            0.0f;
    float const inner_padding_explicit =
        (pad_inner_x && (flags & ImGuiTableFlags_BordersInnerV) != 0) ?
            g.Style.CellPadding.x :
            0.0f;
    ...
    float const outer_padding_for_border =
        (flags & ImGuiTableFlags_BordersOuterV) ? TABLE_BORDER_SIZE : 0.0f;
    float const outer_padding_explicit = pad_outer_x ? g.Style.CellPadding.x :
                                                       0.0f;
    ...
    int const old_columns_count = table->Columns.size();
    ...
    // tested path.
    float const new_ref_scale_unit =
        g.FontSize; // g.Font->GetCharAdvance('A') ?
    if (table->RefScale != 0.0f && table->RefScale != new_ref_scale_unit) {
        float const scale_factor = new_ref_scale_unit / table->RefScale;
...
void ImGui::TableBeginApplyRequests(ImGuiTable* table) {
    ...
    if (table->InstanceCurrent == 0) {
        ...
        if (table->ReorderColumn != -1 && table->ReorderColumnDir != 0) {
            ...
            int const reorder_dir = table->ReorderColumnDir;
            ...
            int const src_order = src_column->DisplayOrder;
            int const dst_order = dst_column->DisplayOrder;
...
static void TableSetupColumnFlags(ImGuiTable* table, ImGuiTableColumn* column,
    ...
    if ((flags & ImGuiTableColumnFlags_WidthMask_) == 0) {
        ImGuiTableFlags const table_sizing_policy =
...
void ImGui::TableUpdateLayout(ImGuiTable* table) {
    ...
    for (int order_n = 0; order_n < table->ColumnsCount; order_n++) {
        ...
        bool const column_is_resizable =
    ...
    ImRect const work_rect = table->WorkRect;
    float const width_spacings = (table->OuterPaddingX * 2.0f) +
                                 (table->CellSpacingX1 + table->CellSpacingX2) *
                                     (table->ColumnsEnabledCount - 1);
    float const width_avail = ((table->Flags & ImGuiTableFlags_ScrollX) &&
                               table->InnerWidth == 0.0f) ?
                                  table->InnerClipRect.GetWidth() :
                                  work_rect.GetWidth();
    float const width_avail_for_stretched_columns =
    ...
    ImRect const mouse_hit_rect(table->OuterRect.Min.x, table->OuterRect.Min.y,
                                table->OuterRect.Max.x,
                                ImMax(table->OuterRect.Max.y,
                                      table->OuterRect.Min.y +
                                          table->LastOuterHeight));
    bool const is_hovering_table = ItemHoverable(mouse_hit_rect, 0);
    ...
    for (int order_n = 0; order_n < table->ColumnsCount; order_n++) {
        ...
        bool const is_visible = column->IsVisibleX; //&& column->IsVisibleY;
...
void ImGui::TableUpdateBorders(ImGuiTable* table) {
    ...
    float const hit_half_width = TABLE_RESIZE_SEPARATOR_HALF_THICKNESS;
    float const hit_y1 = table->OuterRect.Min.y;
    float const hit_y2_body =
        ImMax(table->OuterRect.Max.y, hit_y1 + table->LastOuterHeight);
    float const hit_y2_head = hit_y1 + table->LastFirstRowHeight;
...
void ImGui::EndTable() {
    ...
    ImGuiTableFlags const flags = table->Flags;
    ImGuiWindow* inner_window = table->InnerWindow;
    ...
    float const inner_content_max_y = table->RowPosY2;
             ...
             table->InstanceInteracted == table->InstanceCurrent)
    {
        ...
        // column in sight
        float const neighbor_width_to_keep_visible =
        ...
        table->InstanceCurrent == table->InstanceInteracted)
    {
        ...
        float const new_x2 = (g.IO.MousePos.x - g.ActiveIdClickOffset.x +
    ...
    ImVec2 const backup_outer_max_pos = outer_window->DC.CursorMaxPos;
    ...
    if (table->Flags & ImGuiTableFlags_NoHostExtendX) {
        ...
        float const decoration_size = (table->Flags & ImGuiTableFlags_ScrollX) ?
    ...
    if (temp_data->UserOuterSize.y <= 0.0f) {
        float const decoration_size = (table->Flags & ImGuiTableFlags_ScrollY) ?
...
void ImGui::TableSetupColumn(char const* label, ImGuiTableColumnFlags flags,
...
char const* ImGui::TableGetColumnName(int column_n) {
    ImGuiContext& g = *GImGui;
    ImGuiTable* table = g.CurrentTable;
...
char const* ImGui::TableGetColumnName(ImGuiTable const* table, int column_n) {
    if (table->IsLayoutLocked == false && column_n >= table->DeclColumnsCount)
        return ""; // NameOffset is invalid at this point
    ImGuiTableColumn const* column = &table->Columns[column_n];
...
ImRect ImGui::TableGetCellBgRect(ImGuiTable const* table, int column_n) {
    ImGuiTableColumn const* column = &table->Columns[column_n];
...
ImGuiID ImGui::TableGetColumnResizeID(ImGuiTable const* table, int column_n,
                                      int instance_no) {
    IM_ASSERT(column_n >= 0 && column_n < table->ColumnsCount);
...
void ImGui::TableEndRow(ImGuiTable* table) {
    ...
    // Row background fill
    float const bg_y1 = table->RowPosY1;
    float const bg_y2 = table->RowPosY2;
    bool const unfreeze_rows_actual =
        (table->CurrentRow + 1 == table->FreezeRowsCount);
    bool const unfreeze_rows_request =
        (table->CurrentRow + 1 == table->FreezeRowsRequest);
    if (table->CurrentRow == 0) table->LastFirstRowHeight = bg_y2 - bg_y1;

    bool const is_visible = (bg_y2 >= table->InnerClipRect.Min.y &&
    ...
    if (is_visible) {
        ...
        ImU32 border_col = 0;
        float const border_size = TABLE_BORDER_SIZE;
        ...
        bool const draw_cell_bg_color = table->RowCellDataCurrent >= 0;
        bool const draw_strong_bottom_border = unfreeze_rows_actual;
    ...
    if (unfreeze_rows_actual) {
        ...
        table->DrawSplitter
            ->SetCurrentChannel(window->DrawList,
...
float ImGui::TableGetMaxColumnWidth(ImGuiTable const* table, int column_n) {
    ImGuiTableColumn const* column = &table->Columns[column_n];
    float max_width = FLT_MAX;
    float const min_column_distance =
        table->MinColumnWidth + table->CellPaddingX * 2.0f +
        table->CellSpacingX1 + table->CellSpacingX2;
    if (table->Flags & ImGuiTableFlags_ScrollX) {
...
float ImGui::TableGetColumnWidthAuto(ImGuiTable* table,
                                     ImGuiTableColumn* column) {
    float const content_width_body =
        ImMax(column->ContentMaxXFrozen, column->ContentMaxXUnfrozen) -
        column->WorkMinX;
    float const content_width_headers =
...
void ImGui::TableSetColumnWidth(int column_n, float width) {
    ...
    float const min_width = table->MinColumnWidth;
...
void ImGui::TableSetupDrawChannels(ImGuiTable* table) {
    int const freeze_row_multiplier = (table->FreezeRowsCount > 0) ? 2 : 1;
    int const channels_for_row = (table->Flags & ImGuiTableFlags_NoClip) ?
                                     1 :
                                     table->ColumnsEnabledCount;
    int const channels_for_bg = 1 + 1 * freeze_row_multiplier;
    int const channels_for_dummy =
        (table->ColumnsEnabledCount < table->ColumnsCount ||
         table->VisibleMaskByIndex != table->EnabledMaskByIndex) ?
            +1 :
            0;
    int const channels_total = channels_for_bg +
...
void ImGui::TableMergeDrawChannels(ImGuiTable* table) {
    ImGuiContext& g = *GImGui;
    ImDrawListSplitter* splitter = table->DrawSplitter;
    bool const has_freeze_v = (table->FreezeRowsCount > 0);
    bool const has_freeze_h = (table->FreezeColumnsCount > 0);
    ...
    for (int column_n = 0; column_n < table->ColumnsCount; column_n++) {
        ...
        int const merge_group_sub_count = has_freeze_v ? 2 : 1;
        for (int merge_group_sub_n = 0;
             merge_group_sub_n < merge_group_sub_count; merge_group_sub_n++)
        {
            int const channel_no = (merge_group_sub_n == 0) ?
                                       column->DrawChannelFrozen :
                                       column->DrawChannelUnfrozen;
    ...
    if (merge_group_mask != 0) {
        ...
        int const LEADING_DRAW_CHANNELS = 2;
...
void ImGui::TableDrawBorders(ImGuiTable* table) {
    ...
    float const border_size = TABLE_BORDER_SIZE;
    float const draw_y1 = table->InnerRect.Min.y;
    float const draw_y2_body = table->InnerRect.Max.y;
    float const draw_y2_head =
    ...
    if (table->Flags & ImGuiTableFlags_BordersInnerV) {
        for (int order_n = 0; order_n < table->ColumnsCount; order_n++) {
            ...
            int const column_n = table->DisplayOrderToIndex[order_n];
            ImGuiTableColumn* column = &table->Columns[column_n];
            bool const is_hovered = (table->HoveredColumnBorder == column_n);
            bool const is_resized =
                (table->ResizedColumn == column_n) &&
                (table->InstanceInteracted == table->InstanceCurrent);
            bool const is_resizable =
                (column->Flags & (ImGuiTableColumnFlags_NoResize |
                                  ImGuiTableColumnFlags_NoDirectResize_)) == 0;
            bool const is_frozen_separator =
    ...
    if (table->Flags & ImGuiTableFlags_BordersOuter) {
        ...
        ImRect const outer_border = table->OuterRect;
        ImU32 const outer_col = table->BorderColorStrong;
        if ((table->Flags & ImGuiTableFlags_BordersOuter) ==
            ImGuiTableFlags_BordersOuter)
        {
            ...
            inner_drawlist->AddLine(outer_border.Min,
                                    ImVec2(outer_border.Min.x,
                                           outer_border.Max.y),
                                    outer_col, border_size);
            inner_drawlist->AddLine(ImVec2(outer_border.Max.x,
                                           outer_border.Min.y),
                                    outer_border.Max, outer_col, border_size);
        }
        else if (table->Flags & ImGuiTableFlags_BordersOuterH) {
            inner_drawlist->AddLine(outer_border.Min,
                                    ImVec2(outer_border.Max.x,
                                           outer_border.Min.y),
                                    outer_col, border_size);
            inner_drawlist->AddLine(ImVec2(outer_border.Min.x,
                                           outer_border.Max.y),
                                    outer_border.Max, outer_col, border_size);
        }
    }
        ...
        table->RowPosY2 < table->OuterRect.Max.y)
    {
        ...
        float const border_y = table->RowPosY2;
...
void ImGui::TableSortSpecsSanitize(ImGuiTable* table) {
    ...
    bool const need_fix_linearize = ((ImU64)1 << sort_order_count) !=
                                    (sort_order_mask + 1);
    bool const need_fix_single_sort_order =
...
void ImGui::TableHeadersRow() {
    ...
    // Open row
    float const row_y1 = GetCursorScreenPos().y;
    float const row_height = TableGetHeaderRowHeight();
    TableNextRow(ImGuiTableRowFlags_Headers, row_height);
    if (table->HostSkipItems) // Merely an optimization, you may skip in your
                              // own code.
        return;

    int const columns_count = TableGetColumnCount();
    for (int column_n = 0; column_n < columns_count; column_n++) {
        ...
        char const* name = (TableGetColumnFlags(column_n) &
...
void ImGui::TableHeader(char const* label) {
    ImGuiContext& g = *GImGui;
    ImGuiWindow* window = g.CurrentWindow;
    ...
    int const column_n = table->CurrentColumn;
    ImGuiTableColumn* column = &table->Columns[column_n];
    ...
    if (held || hovered || selected) {
        ImU32 const col = GetColorU32(held    ? ImGuiCol_HeaderActive :
    ...
    // Sort order arrow
    float const ellipsis_max = cell_r.Max.x - w_arrow - w_sort_text;
    if ((table->Flags & ImGuiTableFlags_Sortable) &&
        !(column->Flags & ImGuiTableColumnFlags_NoSort))
    {
        if (column->SortOrder != -1) {
    ...
    bool const text_clipped = label_size.x > (ellipsis_max - label_pos.x);
...
void ImGui::TableDrawContextMenu(ImGuiTable* table) {
    ...
    int const column_n = (table->ContextPopupColumn >= 0 &&
                          table->ContextPopupColumn < table->ColumnsCount) ?
                             table->ContextPopupColumn :
                             -1;
    ImGuiTableColumn* column = (column_n != -1) ? &table->Columns[column_n] :
                                                  NULL;
    ...
    if (table->Flags & ImGuiTableFlags_Resizable) {
        if (column != NULL) {
            bool const can_resize =
                !(column->Flags & ImGuiTableColumnFlags_NoResize) &&
                column->IsEnabled;
            if (MenuItem("Size column to fit###SizeOne", NULL, false,
                         can_resize))
                TableSetColumnWidthAutoSingle(table, column_n);
        }

        char const* size_all_desc;
    ...
    if (table->Flags & ImGuiTableFlags_Hideable) {
             ...
             other_column_n++)
        {
            ...
            char const* name = TableGetColumnName(table, other_column_n);
...
void ImGui::TableLoadSettings(ImGuiTable* table) {
    ...
    ImU64 const expected_display_order_mask =
...
static char const* DebugNodeTableGetSizingPolicyDesc(
...
void ImGui::DebugNodeTable(ImGuiTable* table) {
    ...
    for (int n = 0; n < table->ColumnsCount; n++) {
        ImGuiTableColumn* column = &table->Columns[n];
        char const* name = TableGetColumnName(table, n);
        ImFormatString(
...
float ImGui::GetColumnOffsetFromNorm(ImGuiOldColumns const* columns,
                                     float offset_norm) {
    return offset_norm * (columns->OffMaxX - columns->OffMinX);
}

float ImGui::GetColumnNormFromOffset(ImGuiOldColumns const* columns,
                                     float offset) {
...
void ImGui::BeginColumns(char const* str_id, int columns_count,
    ...
    float const half_clip_extend_x = ImFloor(
        ImMax(window->WindowPadding.x * 0.5f, window->WindowBorderSize));
    float const max_1 = window->WorkRect.Max.x + column_padding -
...
void ImGui::EndColumns() {
    ...
    ImGuiOldColumnFlags const flags = columns->Flags;
    columns->LineMaxY = ImMax(columns->LineMaxY, window->DC.CursorPos.y);
    ...
    if (!(flags & ImGuiOldColumnFlags_NoBorder) && !window->SkipItems) {
        ...
        for (int n = 1; n < columns->Count; n++) {
            ...
            ImGuiID const column_id = columns->ID + ImGuiID(n);
            float const column_hit_hw = COLUMNS_HIT_RECT_HALF_WIDTH;
            ImRect const column_hit_rect(ImVec2(x - column_hit_hw, y1),
                                         ImVec2(x + column_hit_hw, y2));
            KeepAliveID(column_id);
            if (IsClippedEx(column_hit_rect,
                            column_id)) // FIXME: Can be removed or replaced
            ...
            // Draw column
            ImU32 const col = GetColorU32(held    ? ImGuiCol_SeparatorActive :
