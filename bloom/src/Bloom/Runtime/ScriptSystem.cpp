...
//// this is a fallback so we don't fail when getting the methods if no class
/// implements them
...
//		auto handle =
// boxed_cast<EntityHandle>(obj.get_attr("__bloomEntity")); 		return
...
//			engine.registerConstructor<FloatN(expand<float,
// I>...)>(typeName);
//
//			([&]{
//				engine.registerFunction(&getVectorElement<FloatN&, I>,
// std::string("xyzw"[I]));
// engine.registerFunction(&getVectorElement<FloatN const&, I>,
// std::string("xyzw"[I]));
//			}(), ...);
//		});
//	}
...
//
//		engine.registerFunction([](Transform& t) -> auto& { return
// t.position;    }, "position"); 		engine.registerFunction([](Transform& t)
// -> auto& { return t.orientation; }, "orientation");
//		engine.registerFunction([](Transform& t) -> auto& { return
// t.scale;       }, "scale");
//
...
//							   std::string_view
// className, 							   std::string_view fnName)
//	{
//		try {
//			fn();
//			return true;
...
//			BL_LOG(error, "Failed to evaluate {}::{}():\n{}\nReason:
//{}\nDetail: {}\n{}\n", 					 className, fnName,
// e.what(), e.reason, 					 e.detail,
// e.pretty_print());
//		}
//		catch (chaiscript::exception::dispatch_error const& e) {
//			BL_LOG(trace, "class {} has no '{}' method: {}",
//					 className, fnName,
//					 e.what());
//		}
...
//	}
//
//	static void copyFields(auto& object, auto& oldFields) /* mutable lvalue
// because we swap */ { 		for (auto&& [name, value]: object.get_attrs()) {
// if
//(!oldFields.contains(name)) { 				continue;
//			}
...
//		auto& engine = *mEngine;
//		initFn  =
// engine.eval<std::function<void(ScriptObject&)>>("init"); 		updateFn  =
// engine.eval<std::function<void(ScriptObject&, Timestep)>>("update");
...
//
//				std::map<std::string, chaiscript::Boxed_Value>
// oldFields = script.object ? 					script.object->get_attrs() :
// decltype(script.object->get_attrs()){};
//
//				script.object =
// engine.instanciateObject(script.className);
// copyFields(*script.object, oldFields); #warning
////				script.object->get_attr("__bloomEntity") =
/// chaiscript::Boxed_Value(sceneSystem->_scene->getHandle(id));
...
////		assert(sceneSystem && sceneSystem->_scene);
////
////		sceneSystem->_scene->view<ScriptComponent>().each([&](auto id,
