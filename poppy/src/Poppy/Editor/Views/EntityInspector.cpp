...
namespace poppy {
	...
	void EntityInspector::frame() {
		...
		bloom::EntityHandle const entity = selection().empty() ?
			bloom::EntityHandle{} : *selection().entities().begin();
	...
	void EntityInspector::inspectTag(bloom::EntityHandle entity) {
		...
		withFont(FontWeight::bold, FontStyle::roman, [&]{
			...
			float2 const addButtonSize = float2(ImGui::CalcTextSize("Add Component")) + float2(4, 2) * framePadding;
			...
			disabledIf(isSimulating(), [&]{
				...
				ImGui::PushStyleColor(ImGuiCol_FrameBg,        appearance.style().colors.highlightControlFrame);
				ImGui::PushStyleColor(ImGuiCol_FrameBgHovered, appearance.style().colors.highlightControlFrameHovered);
				ImGui::PushStyleVar(ImGuiStyleVar_FramePadding, float2(2, 1) * framePadding);
	...
	void EntityInspector::inspectTransform(bloom::EntityHandle entity) {
		...
		if (beginComponentSection("Transform")) {
			disabledIf(isSimulating(), [&]{
				...
				float2 const lockButtonSize = ImGui::CalcTextSize("").y + style.FramePadding.y * 2;
				
				float const width = ImGui::GetContentRegionAvail().x - lockButtonSize.x - style.ItemSpacing.x;
				...
				if (dragFloat3Pretty("##scale", transform.scale.data(), 0.02) && transformScaleLinked) {
					bool3 const edited = mtl::map(transform.scale, oldScale, utl::unequals);
					if (edited.x) {
				...
				auto const unicode = icons.unicodeStr(transformScaleLinked ? "link" : "unlink");
				ImGui::PushFont(iconFont);
				bool const tsLinked = transformScaleLinked;
				if (tsLinked) {
					ImGui::PushStyleColor(ImGuiCol_Button,
										  appearance.style().colors.highlightControlFrame);
					ImGui::PushStyleColor(ImGuiCol_ButtonHovered,
										  appearance.style().colors.highlightControlFrameHovered);
					ImGui::PushStyleColor(ImGuiCol_ButtonActive,
										  appearance.style().colors.highlightControlFrameDown);
				}
				if (ImGui::Button(unicode.data(), lockButtonSize)) {
					transformScaleLinked ^= true;
				}
				if (tsLinked) { ImGui::PopStyleColor(3); }
				ImGui::PopFont();
			});
			endSection();
		}
	...
	void EntityInspector::inspectLight(bloom::EntityHandle entity) {
		...
		if (beginGenericSection("Light Component", Font::UIDefault().setWeight(FontWeight::semibold), deleter)) {
			disabledIf(isSimulating(), [&]{
				...
				switch (type) {
					...
					case LightType::spotlight:
						...
						break;
					case LightType::directional:
						inspectDirectionalLight(entity.get<DirectionalLightComponent>().light);
						break;
	...
	void EntityInspector::inspectLightType(LightType& type, bloom::EntityHandle entity) {
		using namespace propertiesView;
		beginProperty("Type");
		auto newType = type;
	...
	void EntityInspector::inspectDirectionalLight(bloom::DirectionalLight& light) {
		...
		fullWidth();
		ImGui::DragFloat("shadow-distance-z", &light.shadowDistanceZ, 1, 0, FLT_MAX);
	...
	template <typename T>
	bool EntityInspector::beginComponentSection(std::string_view name, bloom::EntityHandle entity) {
		if constexpr (std::is_same_v<T, void>) {
			return beginGenericSection(name, Font::UIDefault().setWeight(FontWeight::semibold), nullptr);
		}
		else {
			return beginGenericSection(name, Font::UIDefault().setWeight(FontWeight::semibold), [&]{
				entity.remove<T>();
			});
		}
	}
	...
	bool EntityInspector::beginSubSection(std::string_view name) {
		endSection();
		return beginGenericSection(name, Font::UIDefault().setStyle(FontStyle::italic), nullptr);
	}
