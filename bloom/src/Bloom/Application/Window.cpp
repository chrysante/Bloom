...
namespace bloom {
	...
	void Window::onFileDrop(utl::function<void(utl::vector<std::filesystem::path> const&)> f) {
	...
	void Window::onFocus(utl::function<void()> f) {
		onFocusFn = std::move(f);
	}
	...
	void Window::onClose(utl::function<void()> f) {
		onCloseFn = std::move(f);
	}
	
	void Window::onContentScaleChange(utl::function<void(mtl::float2 newContentScale)> f) {
	...
	void Window::setMinSize(mtl::int2 size) {
		...
		glfwSetWindowSizeLimits(GLFW_WND,
								desc.minSize.x > 0 ? desc.minSize.x : GLFW_DONT_CARE,
	...
	void Window::setFocused() {
		glfwFocusWindow(GLFW_WND);
	}
	
	void Window::show() {
		glfwShowWindow(GLFW_WND);
	}
	
	void Window::hide() {
		glfwHideWindow(GLFW_WND);
	}
	
	void Window::maximize() {
		glfwMaximizeWindow(GLFW_WND);
	}
	
	void Window::minimize() {
		glfwIconifyWindow(GLFW_WND);
	}
	
	void Window::restore() {
		glfwRestoreWindow(GLFW_WND);
	}
	...
	void Window::makeFullscreen() {
		...
		glfwSetWindowMonitor(GLFW_WND, monitor,
	...
	void Window::makeWindowed() {
		...
		desc.fullscreen = false;
		glfwSetWindowMonitor(GLFW_WND, nullptr,
	...
	void Window::stopClose() {
		glfwSetWindowShouldClose(GLFW_WND, false);
	}
	
	void Window::requestAttention() {
		glfwRequestWindowAttention(GLFW_WND);
	}
	...
	void Window::setCallbacks() {
		glfwSetMouseButtonCallback(GLFW_WND, [](GLFWwindow* w, int button, int action, int mods) {
			[[maybe_unused]] Window& window = *(Window*)glfwGetWindowUserPointer(w);
			
			window.userInput._mouseButtons[(std::size_t)mouseButtonFromGLFW(button)] = action;
			window.userInput._modFlags = modFlagsFromGLFW(mods);
			
			if (window.onInputFn) {
				auto const event = inputEventFromGLFWMouseButton(window.userInput, button, action, mods);
				window.onInputFn(event);
			}
		});
		
		glfwSetCursorPosCallback(GLFW_WND, [](GLFWwindow* w, double xpos, double ypos) {
			[[maybe_unused]] Window& window = *(Window*)glfwGetWindowUserPointer(w);
			...
			if (window.onInputFn) {
				window.onInputFn(inputEventFromGLFWCursorPos(window.userInput, xpos, ypos));
			}
		});
		...
		glfwSetScrollCallback(GLFW_WND, [](GLFWwindow* w, double xoffset, double yoffset) {
			[[maybe_unused]] Window& window = *(Window*)glfwGetWindowUserPointer(w);
			
			window.userInput._scrollOffset = { xoffset, yoffset };
			
			if (window.onInputFn) {
				window.onInputFn(inputEventFromGLFWScroll(window.userInput, xoffset, yoffset));
			}
		});
	
		glfwSetKeyCallback(GLFW_WND, [](GLFWwindow* w, int key, int scancode, int action, int mods) {
			[[maybe_unused]] Window& window = *(Window*)glfwGetWindowUserPointer(w);
			
			int& repeatCount = window.userInput._keys[(std::size_t)keyFromGLFW(key)];
			switch (action) {
				case GLFW_RELEASE:
					repeatCount = 0;
					break;
			...
			if (window.onInputFn) {
				window.onInputFn(inputEventFromGLFWKey(window.userInput, key, scancode, action, mods));
			}
		});
		...
		glfwSetDropCallback(GLFW_WND, [](GLFWwindow* w, int pathCount, const char *paths[]) {
			[[maybe_unused]] Window& window = *(Window*)glfwGetWindowUserPointer(w);
		...
		glfwSetWindowSizeCallback(GLFW_WND, [](GLFWwindow* w, int sizex, int sizey) {
			[[maybe_unused]] Window& window = *(Window*)glfwGetWindowUserPointer(w);
		...
		glfwSetWindowContentScaleCallback(GLFW_WND, [](GLFWwindow* w, float x, float y) {
			[[maybe_unused]] Window& window = *(Window*)glfwGetWindowUserPointer(w);
	...
	static std::optional<int> calcNewSize(int currentSize, int newSize, int padding) {
		if (newSize <= currentSize && newSize > currentSize - padding) {
			return std::nullopt;
		}
		if (newSize > currentSize) {
			if (padding > 1) {
				while (newSize > currentSize) { currentSize += padding; }
				return currentSize;
			}
			else {
				return newSize;
			...
			if (padding > 1) {
				while (newSize <= currentSize - padding) { currentSize -= padding; }
				return currentSize;
			}
			else {
				return newSize;
	...
	void Window::resizeSwapchain(mtl::int2 newSize) {
		...
		auto newSizeX = calcNewSize(currentSize.x, newSize.x, desc.swapchainResizePadding.x);
		auto newSizeY = calcNewSize(currentSize.y, newSize.y, desc.swapchainResizePadding.y);
		
		if (newSizeX || newSizeY) {
			auto const newSizePadded = int2(newSizeX.value_or(currentSize.x),
											newSizeY.value_or(currentSize.y));
			bloomAssert(newSizePadded.x >= newSize.x);
			bloomAssert(newSizePadded.y >= newSize.y);
