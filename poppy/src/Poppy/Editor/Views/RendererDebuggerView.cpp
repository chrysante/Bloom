...
#include "RendererDebuggerView.hpp"
...
//				ImGui::SliderFloat("##contribution", &params.contribution, 0, 1);
			...
			fullWidth();
			ImGui::SliderInt("##downsample-mipmap-level", &veil.downsampleMipmapLevel, 0,
			...
			auto displayImage = [](TextureView image, int2 size) {
				...
				float2 const imageSize = float2(width, image.size().y / float(image.size().x) * width);
				float2 const cursorEnd = (float2)ImGui::GetCursorPos() + float2(0, imageSize.y + style.ItemSpacing.y);
				window->DrawList->AddImageRounded(image.nativeHandle(),
												  cp, cp + imageSize,
												  {0, 0}, {1, 1}, 0xFFffFFff, style.FrameRounding);
				window->DrawList->AddRect(cp, cp + imageSize, ImGui::GetColorU32(ImGuiCol_Border),
										  style.FrameRounding);
			...
			displayImage(currentFramebuffer->bloom.downsampleMips[veil.downsampleMipmapLevel],
						 currentFramebuffer->size / (1 << (veil.downsampleMipmapLevel + 1)));
			...
			beginProperty("Mipmap Level");
			fullWidth();
			ImGui::SliderInt("##upsample-mipmap-level", &veil.upsampleMipmapLevel, 0,
							 BloomFramebuffer::numUSMipLevels - 1,
							 nullptr, ImGuiSliderFlags_AlwaysClamp);
			endSection();
			
			displayImage(currentFramebuffer->bloom.upsampleMips[veil.upsampleMipmapLevel],
						 currentFramebuffer->size / (1 << (veil.upsampleMipmapLevel + 1)));
		}
	}
	
	bloom::ForwardRenderer* RendererDebuggerView::findRenderer() const {
		if (auto* editorRenderer = dynamic_cast<EditorRenderer*>(&editor().coreSystems().renderer())) {
			 return dynamic_cast<bloom::ForwardRenderer*>(&editorRenderer->wrappedRenderer());
		}
		return dynamic_cast<bloom::ForwardRenderer*>(&editor().coreSystems().renderer());
	}
	...
	std::shared_ptr<bloom::ForwardRendererFramebuffer> RendererDebuggerView::findFramebuffer() const {
		...
		for (auto* const viewport: utl::transform_range(views, [](View* view) { return dynamic_cast<Viewport*>(view); })) {
			...
			auto const fwFramebuffer = std::dynamic_pointer_cast<bloom::ForwardRendererFramebuffer>(viewport->framebuffer);
			if (fwFramebuffer) {
