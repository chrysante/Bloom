...
//	static mtl::float4x4 directionalLightSpaceTransform(Camera const&
// camera, float dist, mtl::float3 lightDirection) { 		using namespace mtl;
//
...
//		float3 const centerFar = camera.position() + camera.front() *
// farDist;
//
//		float3 const topLeftFar     = centerFar + (camera.up() * Hfar / 2)
//- (camera.right() * Wfar / 2); 		float3 const topRightFar    = centerFar
//+ (camera.up() * Hfar / 2) + (camera.right() * Wfar / 2); 		float3 const
// bottomLeftFar  = centerFar - (camera.up() * Hfar / 2) - (camera.right() *
// Wfar
/// 2); 		float3 const bottomRightFar = centerFar - (camera.up() * Hfar /
/// 2) +
//(camera.right() * Wfar / 2);
//
//		float3 const centerNear = camera.position() + camera.front() *
// nearDist;
//
//		float3 const topLeftNear     = centerNear + (camera.up() * Hnear /
// 2) - (camera.right() * Wnear / 2); 		float3 const topRightNear    =
// centerNear + (camera.up() * Hnear / 2) + (camera.right() * Wnear / 2);
// float3 const bottomLeftNear  = centerNear - (camera.up() * Hnear / 2) -
// (camera.right() *
// Wnear / 2); 		float3 const bottomRightNear = centerNear - (camera.up() *
// Hnear / 2) + (camera.right() * Wnear / 2);
//
//		float3 const frustumCenter = (centerFar- centerNear) * 0.5f;
//
//		float4x4 const lightView =
// mtl::look_at<mtl::right_handed>(mtl::normalize(lightDirection),
...
//		// towards us so need to set it as the near plane flipped same for
// min.z. 		float const n = -max.z; 		float const f = -min.z;
//
//		// finally, set our ortho projection
//		// and create the light space view-projection matrix
//		mtl::float4x4 const lightProjection =
// mtl::ortho<mtl::right_handed>(l,r,b,t,n,f); 		mtl::float4x4 const
...
void Renderer::submit(Reference<StaticRenderMesh> mesh,
...
//	void Renderer::submitSelected(Reference<StaticRenderMesh> mesh,
// Reference<Material> material, EntityRenderData entityData) {
//		bloomExpect(buildingScene, "Forgot to call beginScene?");
//		entityData.transform = mtl::transpose(entityData.transform);
//		selectedObjects.push_back({ entityData, std::move(material),
// std::move(mesh) });
//	}
...
void Renderer::drawWireframe(Framebuffer& framebuffer,
...
void Renderer::drawSelection(EditorFramebuffer& framebuffer,
                             CommandQueue& commandQueue) {
...
void Renderer::wireframePass(Framebuffer& framebuffer,
                             CommandQueue& commandQueue) {
    ...
    Material* currentMaterial = nullptr;
    for (std::size_t index = 0; auto&& object: objects) {
        utl_defer { ++index; };
        if (object.material.get() != currentMaterial) {
            currentMaterial = object.material.get();
...
void Renderer::shadowMapPass(CommandQueue& commandQueue) {
    ...
    std::size_t const numShadowMaps =
        std::accumulate(numCascades.begin(), numCascades.end(), 0);
...
void Renderer::uploadShadowData() {
    ...
    std::size_t const size =
        sizeof(ShadowRenderData) +
        std::max(lightSpaceTransforms.size(), std::size_t{ 1 }) *
            sizeof(mtl::float4x4);
