...
bloom::Application* bloom::createApplication() {
	std::cerr << "Error: createApplication() must be overridden by client" << std::endl;
	std::terminate();
}
...
namespace bloom {
										...
										std::unique_ptr<WindowDelegate> delegate)
	{
		...
		window->onResizePrivateFn = [this](int2 newSize) {
			doFrame();
		};
		
		window->onMovePrivateFn = [this](int2 newSize) {
			doFrame();
		};
