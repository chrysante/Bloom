...
namespace poppy {
	...
	void MaterialInstanceViewer::frame() {
		...
		auto materialName = inst->material() ? inst->material()->name() : "No Material Assigned";
		ImGui::Button(materialName.data(), { ImGui::GetContentRegionAvail().x, 30 });
		if (auto const assetHandle = acceptAssetDragDrop(bloom::AssetType::material)) {
		...
		if (beginSection()) {
			...
			beginProperty("Metallic"); fullWidth();
			edited |= ImGui::SliderFloat("##metallic", &params.metallic, 0, 1);
			beginProperty("Roughness"); fullWidth();
			...
			edited |= ImGui::ColorEdit3("##emissive", params.emissive.data(),
										...
										ImGuiColorEditFlags_NoAlpha |
										ImGuiColorEditFlags_PickerHueWheel);
			beginProperty("Emissive Intensity"); fullWidth();
			edited |= ImGui::DragFloat("##emissive-intensity", &params.emissiveIntensity, .1);
	...
	void MaterialInstanceViewer::setMaterialInstance(bloom::Reference<MaterialInstance> _) {
		inst = std::move(_);
		editor().coreSystems().assetManager().makeAvailable(inst->handle(), AssetRepresentation::CPU | AssetRepresentation::GPU);
