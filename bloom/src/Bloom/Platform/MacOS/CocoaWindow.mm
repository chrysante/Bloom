...
static InputEvent makeInputEvent(InputEventMask type,
                                 MouseButton button,
                                 bool isDown,
                                 int clickCount,
                                 Input const& input) {
...
static bool isOverWindowButton(NSEvent* event, NSWindowButton button) {
    ...
    buttonFrame.origin.y = windowFrame.size.height - buttonFrame.origin.y - buttonFrame.size.height;
    return NSPointInRect(event.locationInWindow, buttonFrame);
}

/// \Returns `true` if \p value is in the interval `[min + padding, max - padding]`
static bool isInBounds(double value, double min, double max, double padding) {
    return value >= min + padding && value <= max - padding;
}

/// \Returns `true` if the location of \p event is within the area of the window where the window is resizeable, i.e. in any window location where the mouse image changes to a resize indicator.
/// This is used to avoid starting window drag operations when resizing
...
enum class CocoaEventType: short {
    WindowShallZoom
};
...
void Window::platformInit() {
    ...
    [NSEvent addLocalMonitorForEventsMatchingMask:mask handler:^NSEvent*(NSEvent* event) {
        ...
        if (callbacks.onInputFn) {
            auto inputEvent = makeInputEvent(translateType(event.type),
                                             button,
                                             isDown,
