...
namespace poppy {
	...
	void EditorSceneRenderer::submitExtra() {
		...
		for (auto entity: mSelection->entities()) {
			...
			editorRenderer->submitSelected(meshRenderer.mesh->getRenderer(),
										   entity.get<TransformMatrixComponent>().matrix);
		}
	}
	
	void EditorSceneRenderer::drawWithOverlays(std::span<Scene const* const> scenes,
											   ...
											   CommandQueue& commandQueue)
	{
		...
		if (auto* const editorRenderer = dynamic_cast<EditorRenderer*>(&renderer())) {
			editorRenderer->drawOverlays(framebuffer, editorFramebuffer, commandQueue, drawDesc);
		}
	}
