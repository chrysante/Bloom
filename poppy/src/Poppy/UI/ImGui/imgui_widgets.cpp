...
static signed char const IM_S8_MIN = -128;
static signed char const IM_S8_MAX = 127;
static unsigned char const IM_U8_MIN = 0;
static unsigned char const IM_U8_MAX = 0xFF;
static signed short const IM_S16_MIN = -32768;
static signed short const IM_S16_MAX = 32767;
static unsigned short const IM_U16_MIN = 0;
static unsigned short const IM_U16_MAX = 0xFFFF;
static ImS32 const IM_S32_MIN = INT_MIN; // (-2147483647 - 1), (0x80000000);
static ImS32 const IM_S32_MAX = INT_MAX; // (2147483647), (0x7FFFFFFF)
static ImU32 const IM_U32_MIN = 0;
static ImU32 const IM_U32_MAX = UINT_MAX; // (0xFFFFFFFF)
#ifdef LLONG_MIN
static ImS64 const IM_S64_MIN = LLONG_MIN; // (-9223372036854775807ll - 1ll);
static ImS64 const IM_S64_MAX = LLONG_MAX; // (9223372036854775807ll);
#else
static ImS64 const IM_S64_MIN = -9223372036854775807LL - 1;
static ImS64 const IM_S64_MAX = 9223372036854775807LL;
#endif
static ImU64 const IM_U64_MIN = 0;
#ifdef ULLONG_MAX
static ImU64 const IM_U64_MAX = ULLONG_MAX; // (0xFFFFFFFFFFFFFFFFull);
#else
static ImU64 const IM_U64_MAX = (2ULL * 9223372036854775807LL + 1);
#endif
...
static int InputTextCalcTextLenAndLineCount(char const* text_begin,
                                            char const** out_text_end);
static ImVec2 InputTextCalcTextSizeW(ImWchar const* text_begin,
                                     ImWchar const* text_end,
                                     ImWchar const** remaining = NULL,
...
void ImGui::TextEx(char const* text, char const* text_end,
    ...
    ImVec2 const text_pos(window->DC.CursorPos.x,
...
void ImGui::TextColored(ImVec4 const& col, char const* fmt, ...) {
    va_list args;
    va_start(args, fmt);
...
void ImGui::TextColoredV(ImVec4 const& col, char const* fmt, va_list args) {
    PushStyleColor(ImGuiCol_Text, col);
    if (fmt[0] == '%' && fmt[1] == 's' && fmt[2] == 0)
        TextEx(va_arg(args, char const*), NULL,
...
void ImGui::LabelTextV(char const* label, char const* fmt, va_list args) {
    ...
    char const* value_text_end =
        value_text_begin +
        ImFormatStringV(g.TempBuffer, IM_ARRAYSIZE(g.TempBuffer), fmt, args);
    ImVec2 const value_size =
        CalcTextSize(value_text_begin, value_text_end, false);
    ImVec2 const label_size = CalcTextSize(label, NULL, true);

    ImVec2 const pos = window->DC.CursorPos;
    ImRect const value_bb(pos, pos + ImVec2(w, value_size.y +
                                                   style.FramePadding.y * 2));
    ImRect const total_bb(pos, pos + ImVec2(w + (label_size.x > 0.0f ?
...
void ImGui::BulletTextV(char const* fmt, va_list args) {
    ...
    char const* text_end =
        text_begin +
        ImFormatStringV(g.TempBuffer, IM_ARRAYSIZE(g.TempBuffer), fmt, args);
    ImVec2 const label_size = CalcTextSize(text_begin, text_end, false);
    ImVec2 const total_size =
    ...
    ImRect const bb(pos, pos + total_size);
    if (!ItemAdd(bb, 0)) return;
...
bool ImGui::ButtonBehavior(ImRect const& bb, ImGuiID id, bool* out_hovered,
...
bool ImGui::ButtonEx(char const* label, ImVec2 const& size_arg,
    ...
    // Render
    ImU32 const col = GetColorU32((held && hovered) ? ImGuiCol_ButtonActive :
...
bool ImGui::Button(char const* label, ImVec2 const& size_arg) {
...
bool ImGui::InvisibleButton(char const* str_id, ImVec2 const& size_arg,
                            ImGuiButtonFlags flags) {
    ImGuiContext& g = *GImGui;
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems) return false;
    ...
    ImRect const bb(window->DC.CursorPos, window->DC.CursorPos + size);
    ItemSize(size);
    if (!ItemAdd(bb, id)) return false;
...
bool ImGui::ArrowButtonEx(char const* str_id, ImGuiDir dir, ImVec2 size,
                          ImGuiButtonFlags flags) {
    ImGuiContext& g = *GImGui;
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems) return false;
    ...
    // Render
    ImU32 const bg_col = GetColorU32((held && hovered) ? ImGuiCol_ButtonActive :
                                     hovered ? ImGuiCol_ButtonHovered :
                                               ImGuiCol_Button);
    ImU32 const text_col = GetColorU32(ImGuiCol_Text);
    RenderNavHighlight(bb, id);
...
bool ImGui::CloseButton(ImGuiID id, ImVec2 const& pos) {
    ImGuiContext& g = *GImGui;
    ...
    // always accessible?
    ImRect const bb(pos, pos + ImVec2(g.FontSize, g.FontSize) +
                             g.Style.FramePadding * 2.0f);
    ImRect bb_interact = bb;
    float const area_to_visible_ratio =
    ...
    ImVec2 const size = bb.GetSize();
    ImVec2 const close_button_padding = size * 0.125;
    float const close_button_rounding = ImMin(size.x, size.y) * .125;
    ImVec2 center = bb.GetCenter();
    if (hovered) {
#if POPPY_CUSTOM
        window->DrawList->AddRectFilled(bb.Min + close_button_padding,
                                        bb.Max - close_button_padding, col,
                                        close_button_rounding);
#else
        window->DrawList->AddCircleFilled(center,
                                          ImMax(2.0f, g.FontSize * 0.5f + 1.0f),
                                          col, 12);
#endif
    }
...
bool ImGui::CollapseButton(ImGuiID id, ImVec2 const& pos,
...
ImRect ImGui::GetWindowScrollbarRect(ImGuiWindow* window, ImGuiAxis axis) {
    ImRect const outer_rect = window->Rect();
    ImRect const inner_rect = window->InnerRect;
    float const border_size = window->WindowBorderSize;
    float const scrollbar_size =
...
void ImGui::Scrollbar(ImGuiAxis axis) {
    ...
    ImGuiID const id = GetWindowScrollbarID(window, axis);
    KeepAliveID(id);
...
bool ImGui::ScrollbarEx(ImRect const& bb_frame, ImGuiID id, ImGuiAxis axis,
    ...
    bb.Expand(
        ImVec2(-ImClamp(IM_FLOOR((bb_frame_width - 2.0f) * 0.5f), 0.0f, 3.0f),
               -ImClamp(IM_FLOOR((bb_frame_height - 2.0f) * 0.5f), 0.0f,
                        3.0f)));

    // V denote the main, longer axis of the scrollbar (= height for a vertical
    // scrollbar)
    float const scrollbar_size_v = (axis == ImGuiAxis_X) ? bb.GetWidth() :
                                                           bb.GetHeight();
    ...
    ImS64 const scroll_max = ImMax((ImS64)1, size_contents_v - size_avail_v);
    ...
    if (held && allow_interaction && grab_h_norm < 1.0f) {
        float const scrollbar_pos_v = bb.Min[axis];
        float const mouse_pos_v = g.IO.MousePos[axis];

        // Click position in scrollbar normalized space (0.0f->1.0f)
        float const clicked_v_norm =
    ...
    // Render
    ImU32 const bg_col = GetColorU32(ImGuiCol_ScrollbarBg);
    ImU32 const grab_col = GetColorU32(held    ? ImGuiCol_ScrollbarGrabActive :
...
void ImGui::Image(ImTextureID user_texture_id, ImVec2 const& size,
                  ImVec2 const& uv0, ImVec2 const& uv1, ImVec4 const& tint_col,
...
bool ImGui::ImageButtonEx(ImGuiID id, ImTextureID texture_id,
                          ImVec2 const& size, ImVec2 const& uv0,
                          ImVec2 const& uv1, ImVec2 const& padding,
                          ImVec4 const& bg_col, ImVec4 const& tint_col) {
    ImGuiContext& g = *GImGui;
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems) return false;
...
bool ImGui::ImageButton(ImTextureID user_texture_id, ImVec2 const& size,
                        ImVec2 const& uv0, ImVec2 const& uv1, int frame_padding,
                        ImVec4 const& bg_col, ImVec4 const& tint_col) {
    ImGuiContext& g = *GImGui;
    ImGuiWindow* window = g.CurrentWindow;
    if (window->SkipItems) return false;
    ...
    ImGuiID const id = window->GetID("#image");
    PopID();

    ImVec2 const padding = (frame_padding >= 0) ? ImVec2((float)frame_padding,
                                                         (float)frame_padding) :
...
bool ImGui::Checkbox(char const* label, bool* v) {
    ...
    ImVec2 const pos = window->DC.CursorPos;
    ImRect const total_bb(pos, pos + ImVec2(square_sz +
                                                (label_size.x > 0.0f ?
                                                     style.ItemInnerSpacing.x +
                                                         label_size.x :
                                                     0.0f),
    ...
    bool mixed_value = (g.LastItemData.InFlags & ImGuiItemFlags_MixedValue) !=
                       0;
    if (mixed_value) {
...
template <typename T>
bool ImGui::CheckboxFlagsT(char const* label, T* flags, T flags_value) {
...
bool ImGui::CheckboxFlags(char const* label, int* flags, int flags_value) {
    return CheckboxFlagsT(label, flags, flags_value);
}

bool ImGui::CheckboxFlags(char const* label, unsigned int* flags,
                          unsigned int flags_value) {
    return CheckboxFlagsT(label, flags, flags_value);
}

bool ImGui::CheckboxFlags(char const* label, ImS64* flags, ImS64 flags_value) {
    return CheckboxFlagsT(label, flags, flags_value);
}

bool ImGui::CheckboxFlags(char const* label, ImU64* flags, ImU64 flags_value) {
    return CheckboxFlagsT(label, flags, flags_value);
}
...
// size
void ImGui::ProgressBar(float fraction, ImVec2 const& size_arg,
                        char const* overlay) {
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems) return;
...
void ImGui::Dummy(ImVec2 const& size) {
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems) return;
...
void ImGui::NewLine() {
    ...
    ImGuiContext& g = *GImGui;
    ImGuiLayoutType const backup_layout_type = window->DC.LayoutType;
...
void ImGui::SeparatorEx(ImGuiSeparatorFlags flags) {
    ...
    if (flags & ImGuiSeparatorFlags_Vertical) {
        ...
        ImRect const bb(ImVec2(window->DC.CursorPos.x, y1),
        ...
        ImRect const bb(ImVec2(x1, window->DC.CursorPos.y),
...
bool ImGui::SplitterBehavior(ImRect const& bb, ImGuiID id, ImGuiAxis axis,
    ...
    g.CurrentItemFlags |= ImGuiItemFlags_NoNav |
...
static int IMGUI_CDECL ShrinkWidthItemComparer(void const* lhs,
                                               void const* rhs) {
    ImGuiShrinkWidthItem const* a = (ImGuiShrinkWidthItem const*)lhs;
    ImGuiShrinkWidthItem const* b = (ImGuiShrinkWidthItem const*)rhs;
...
bool ImGui::BeginCombo(char const* label, char const* preview_value,
    ...
    float const arrow_size =
        (flags & ImGuiComboFlags_NoArrowButton) ? 0.0f : GetFrameHeight();
    ImVec2 const label_size = CalcTextSize(label, NULL, true);
    float const w = (flags & ImGuiComboFlags_NoPreview) ? arrow_size :
                                                          CalcItemWidth();
    ImRect const bb(window->DC.CursorPos,
                    window->DC.CursorPos +
                        ImVec2(w, label_size.y + style.FramePadding.y * 2.0f));
    ImRect const total_bb(bb.Min, bb.Max + ImVec2(label_size.x > 0.0f ?
                                                      style.ItemInnerSpacing.x +
                                                          label_size.x :
                                                      0.0f,
                                                  0.0f));
    ...
    // Render shape
    ImU32 const frame_col =
        GetColorU32(hovered ? ImGuiCol_FrameBgHovered : ImGuiCol_FrameBg);
    float const value_x2 = ImMax(bb.Min.x, bb.Max.x - arrow_size);
    RenderNavHighlight(bb, id);
...
bool ImGui::BeginComboPopup(ImGuiID popup_id, ImRect const& bb,
                            ImGuiComboFlags flags) {
    ImGuiContext& g = *GImGui;
    ...
    if (ImGuiWindow* popup_window = FindWindowByName(name))
        if (popup_window->WasActive) {
            ...
            ImVec2 pos =
                FindBestWindowPosForPopupEx(bb.GetBL(), size_expected,
                                            &popup_window->AutoPosLastDirection,
                                            r_outer, bb,
    ...
    PushStyleVar(ImGuiStyleVar_WindowPadding,
                 ImVec2(g.Style.FramePadding.x,
                        g.Style.WindowPadding
                            .y)); // Horizontally align ourselves with the
                                  // framed text
...
bool ImGui::Combo(char const* label, int* current_item,
                  bool (*items_getter)(void*, int, char const**), void* data,
...
bool ImGui::Combo(char const* label, int* current_item,
                  char const* const items[], int items_count,
                  int height_in_items) {
    bool const value_changed = Combo(label, current_item, Items_ArrayGetter,
...
bool ImGui::Combo(char const* label, int* current_item,
                  char const* items_separated_by_zeros, int height_in_items) {
    int items_count = 0;
    char const* p =
...
static ImGuiDataTypeInfo const GDataTypeInfo[] = {
...
static char const* PatchFormatStringFloatToInt(char const* fmt) {
    ...
    char const* fmt_end = ImParseFormatFindEnd(
...
ImGuiDataTypeInfo const* ImGui::DataTypeGetInfo(ImGuiDataType data_type) {
    IM_ASSERT(data_type >= 0 && data_type < ImGuiDataType_COUNT);
    return &GDataTypeInfo[data_type];
}

int ImGui::DataTypeFormatString(char* buf, int buf_size,
                                ImGuiDataType data_type, void const* p_data,
                                char const* format) {
    // Signedness doesn't matter when pushing integer arguments
    if (data_type == ImGuiDataType_S32 || data_type == ImGuiDataType_U32)
        return ImFormatString(buf, buf_size, format, *(ImU32 const*)p_data);
    if (data_type == ImGuiDataType_S64 || data_type == ImGuiDataType_U64)
        return ImFormatString(buf, buf_size, format, *(ImU64 const*)p_data);
    ...
    if (data_type == ImGuiDataType_S8)
        return ImFormatString(buf, buf_size, format, *(ImS8 const*)p_data);
    if (data_type == ImGuiDataType_U8)
        return ImFormatString(buf, buf_size, format, *(ImU8 const*)p_data);
    if (data_type == ImGuiDataType_S16)
        return ImFormatString(buf, buf_size, format, *(ImS16 const*)p_data);
    if (data_type == ImGuiDataType_U16)
        return ImFormatString(buf, buf_size, format, *(ImU16 const*)p_data);
    IM_ASSERT(0);
    return 0;
}

void ImGui::DataTypeApplyOp(ImGuiDataType data_type, int op, void* output,
                            void const* arg1, void const* arg2) {
    IM_ASSERT(op == '+' || op == '-');
    switch (data_type) {
    case ImGuiDataType_S8:
        if (op == '+') {
            *(ImS8*)output = ImAddClampOverflow(*(ImS8 const*)arg1,
                                                *(ImS8 const*)arg2, IM_S8_MIN,
                                                IM_S8_MAX);
        }
        if (op == '-') {
            *(ImS8*)output = ImSubClampOverflow(*(ImS8 const*)arg1,
                                                *(ImS8 const*)arg2, IM_S8_MIN,
                                                IM_S8_MAX);
        }
        return;
    case ImGuiDataType_U8:
        if (op == '+') {
            *(ImU8*)output = ImAddClampOverflow(*(ImU8 const*)arg1,
                                                *(ImU8 const*)arg2, IM_U8_MIN,
                                                IM_U8_MAX);
        }
        if (op == '-') {
            *(ImU8*)output = ImSubClampOverflow(*(ImU8 const*)arg1,
                                                *(ImU8 const*)arg2, IM_U8_MIN,
                                                IM_U8_MAX);
        }
        return;
    case ImGuiDataType_S16:
        if (op == '+') {
            *(ImS16*)output = ImAddClampOverflow(*(ImS16 const*)arg1,
                                                 *(ImS16 const*)arg2,
                                                 IM_S16_MIN, IM_S16_MAX);
        }
        if (op == '-') {
            *(ImS16*)output = ImSubClampOverflow(*(ImS16 const*)arg1,
                                                 *(ImS16 const*)arg2,
                                                 IM_S16_MIN, IM_S16_MAX);
        }
        return;
    case ImGuiDataType_U16:
        if (op == '+') {
            *(ImU16*)output = ImAddClampOverflow(*(ImU16 const*)arg1,
                                                 *(ImU16 const*)arg2,
                                                 IM_U16_MIN, IM_U16_MAX);
        }
        if (op == '-') {
            *(ImU16*)output = ImSubClampOverflow(*(ImU16 const*)arg1,
                                                 *(ImU16 const*)arg2,
                                                 IM_U16_MIN, IM_U16_MAX);
        }
        return;
    case ImGuiDataType_S32:
        if (op == '+') {
            *(ImS32*)output = ImAddClampOverflow(*(ImS32 const*)arg1,
                                                 *(ImS32 const*)arg2,
                                                 IM_S32_MIN, IM_S32_MAX);
        }
        if (op == '-') {
            *(ImS32*)output = ImSubClampOverflow(*(ImS32 const*)arg1,
                                                 *(ImS32 const*)arg2,
                                                 IM_S32_MIN, IM_S32_MAX);
        }
        return;
    case ImGuiDataType_U32:
        if (op == '+') {
            *(ImU32*)output = ImAddClampOverflow(*(ImU32 const*)arg1,
                                                 *(ImU32 const*)arg2,
                                                 IM_U32_MIN, IM_U32_MAX);
        }
        if (op == '-') {
            *(ImU32*)output = ImSubClampOverflow(*(ImU32 const*)arg1,
                                                 *(ImU32 const*)arg2,
                                                 IM_U32_MIN, IM_U32_MAX);
        }
        return;
    case ImGuiDataType_S64:
        if (op == '+') {
            *(ImS64*)output = ImAddClampOverflow(*(ImS64 const*)arg1,
                                                 *(ImS64 const*)arg2,
                                                 IM_S64_MIN, IM_S64_MAX);
        }
        if (op == '-') {
            *(ImS64*)output = ImSubClampOverflow(*(ImS64 const*)arg1,
                                                 *(ImS64 const*)arg2,
                                                 IM_S64_MIN, IM_S64_MAX);
        }
        return;
    case ImGuiDataType_U64:
        if (op == '+') {
            *(ImU64*)output = ImAddClampOverflow(*(ImU64 const*)arg1,
                                                 *(ImU64 const*)arg2,
                                                 IM_U64_MIN, IM_U64_MAX);
        }
        if (op == '-') {
            *(ImU64*)output = ImSubClampOverflow(*(ImU64 const*)arg1,
                                                 *(ImU64 const*)arg2,
                                                 IM_U64_MIN, IM_U64_MAX);
        }
        return;
    case ImGuiDataType_Float:
        if (op == '+') {
            *(float*)output = *(float const*)arg1 + *(float const*)arg2;
        }
        if (op == '-') {
            *(float*)output = *(float const*)arg1 - *(float const*)arg2;
        }
    ...
    case ImGuiDataType_Double:
        if (op == '+') {
            *(double*)output = *(double const*)arg1 + *(double const*)arg2;
        }
        if (op == '-') {
            *(double*)output = *(double const*)arg1 - *(double const*)arg2;
        }
...
bool ImGui::DataTypeApplyFromText(char const* buf, ImGuiDataType data_type,
                                  void* p_data, char const* format) {
    ...
    ImGuiDataTypeInfo const* type_info = DataTypeGetInfo(data_type);
    ...
    {
        // For float/double we have to ignore format with precision (e.g.
        // "%.2f") because sscanf doesn't take them in, so force them into %f
        // and %lf
        if (data_type == ImGuiDataType_Float ||
...
template <typename T>
static int DataTypeCompareT(T const* lhs, T const* rhs) {
    if (*lhs < *rhs) return -1;
    if (*lhs > *rhs) return +1;
    return 0;
}

int ImGui::DataTypeCompare(ImGuiDataType data_type, void const* arg_1,
                           void const* arg_2) {
    switch (data_type) {
    case ImGuiDataType_S8:
        return DataTypeCompareT<ImS8>((ImS8 const*)arg_1, (ImS8 const*)arg_2);
    case ImGuiDataType_U8:
        return DataTypeCompareT<ImU8>((ImU8 const*)arg_1, (ImU8 const*)arg_2);
    case ImGuiDataType_S16:
        return DataTypeCompareT<ImS16>((ImS16 const*)arg_1,
                                       (ImS16 const*)arg_2);
    case ImGuiDataType_U16:
        return DataTypeCompareT<ImU16>((ImU16 const*)arg_1,
                                       (ImU16 const*)arg_2);
    case ImGuiDataType_S32:
        return DataTypeCompareT<ImS32>((ImS32 const*)arg_1,
                                       (ImS32 const*)arg_2);
    case ImGuiDataType_U32:
        return DataTypeCompareT<ImU32>((ImU32 const*)arg_1,
                                       (ImU32 const*)arg_2);
    case ImGuiDataType_S64:
        return DataTypeCompareT<ImS64>((ImS64 const*)arg_1,
                                       (ImS64 const*)arg_2);
    case ImGuiDataType_U64:
        return DataTypeCompareT<ImU64>((ImU64 const*)arg_1,
                                       (ImU64 const*)arg_2);
    case ImGuiDataType_Float:
        return DataTypeCompareT<float>((float const*)arg_1,
                                       (float const*)arg_2);
    case ImGuiDataType_Double:
        return DataTypeCompareT<double>((double const*)arg_1,
                                        (double const*)arg_2);
    case ImGuiDataType_COUNT:
        break;
    }
...
template <typename T>
static bool DataTypeClampT(T* v, T const* v_min, T const* v_max) {
...
bool ImGui::DataTypeClamp(ImGuiDataType data_type, void* p_data,
    ...
    switch (data_type) {
    case ImGuiDataType_S8:
        return DataTypeClampT<ImS8>((ImS8*)p_data, (ImS8 const*)p_min,
                                    (ImS8 const*)p_max);
    case ImGuiDataType_U8:
        return DataTypeClampT<ImU8>((ImU8*)p_data, (ImU8 const*)p_min,
                                    (ImU8 const*)p_max);
    case ImGuiDataType_S16:
        return DataTypeClampT<ImS16>((ImS16*)p_data, (ImS16 const*)p_min,
                                     (ImS16 const*)p_max);
    case ImGuiDataType_U16:
        return DataTypeClampT<ImU16>((ImU16*)p_data, (ImU16 const*)p_min,
                                     (ImU16 const*)p_max);
    case ImGuiDataType_S32:
        return DataTypeClampT<ImS32>((ImS32*)p_data, (ImS32 const*)p_min,
                                     (ImS32 const*)p_max);
    case ImGuiDataType_U32:
        return DataTypeClampT<ImU32>((ImU32*)p_data, (ImU32 const*)p_min,
                                     (ImU32 const*)p_max);
    case ImGuiDataType_S64:
        return DataTypeClampT<ImS64>((ImS64*)p_data, (ImS64 const*)p_min,
                                     (ImS64 const*)p_max);
    case ImGuiDataType_U64:
        return DataTypeClampT<ImU64>((ImU64*)p_data, (ImU64 const*)p_min,
                                     (ImU64 const*)p_max);
    case ImGuiDataType_Float:
        return DataTypeClampT<float>((float*)p_data, (float const*)p_min,
                                     (float const*)p_max);
    case ImGuiDataType_Double:
        return DataTypeClampT<double>((double*)p_data, (double const*)p_min,
                                      (double const*)p_max);
    case ImGuiDataType_COUNT:
        break;
    }
...
static float GetMinimumStepAtDecimalPrecision(int decimal_precision) {
    static float const min_steps[10] = { 1.0f,        0.1f,       0.01f,
...
static void SanitizeFormatString(char const* fmt, char* fmt_out,
                                 size_t fmt_out_size) {
    IM_UNUSED(fmt_out_size);
    char const* fmt_end = ImParseFormatFindEnd(fmt);
...
TYPE ImGui::RoundScalarWithFormatT(char const* format, ImGuiDataType data_type,
                                   TYPE v) {
...
bool ImGui::DragBehaviorT(ImGuiDataType data_type, TYPE* v, float v_speed,
                          ...
                          char const* format, ImGuiSliderFlags flags) {
    ImGuiContext& g = *GImGui;
    ImGuiAxis const axis = (flags & ImGuiSliderFlags_Vertical) ? ImGuiAxis_Y :
                                                                 ImGuiAxis_X;
    bool const is_clamped = (v_min < v_max);
    bool const is_logarithmic = (flags & ImGuiSliderFlags_Logarithmic) != 0;
    bool const is_floating_point = (data_type == ImGuiDataType_Float) ||
                                   (data_type == ImGuiDataType_Double);
    ...
    float const zero_deadzone_halfsize =
...
bool ImGui::DragBehavior(ImGuiID id, ImGuiDataType data_type, void* p_v,
                         float v_speed, void const* p_min, void const* p_max,
                         char const* format, ImGuiSliderFlags flags) {
    ...
    case ImGuiDataType_S8: {
        ...
        bool r =
            DragBehaviorT<ImS32, ImS32, float>(ImGuiDataType_S32, &v32, v_speed,
                                               p_min ? *(ImS8 const*)p_min :
                                                       IM_S8_MIN,
                                               p_max ? *(ImS8 const*)p_max :
                                                       IM_S8_MAX,
                                               format, flags);
    ...
    case ImGuiDataType_U8: {
        ...
        bool r =
            DragBehaviorT<ImU32, ImS32, float>(ImGuiDataType_U32, &v32, v_speed,
                                               p_min ? *(ImU8 const*)p_min :
                                                       IM_U8_MIN,
                                               p_max ? *(ImU8 const*)p_max :
                                                       IM_U8_MAX,
                                               format, flags);
    ...
    case ImGuiDataType_S16: {
        ...
        bool r =
            DragBehaviorT<ImS32, ImS32, float>(ImGuiDataType_S32, &v32, v_speed,
                                               p_min ? *(ImS16 const*)p_min :
                                                       IM_S16_MIN,
                                               p_max ? *(ImS16 const*)p_max :
                                                       IM_S16_MAX,
                                               format, flags);
    ...
    case ImGuiDataType_U16: {
        ...
        bool r =
            DragBehaviorT<ImU32, ImS32, float>(ImGuiDataType_U32, &v32, v_speed,
                                               p_min ? *(ImU16 const*)p_min :
                                                       IM_U16_MIN,
                                               p_max ? *(ImU16 const*)p_max :
                                                       IM_U16_MAX,
                                               format, flags);
    ...
    case ImGuiDataType_S32:
        return DragBehaviorT<ImS32, ImS32, float>(data_type, (ImS32*)p_v,
                                                  v_speed,
                                                  p_min ? *(ImS32 const*)p_min :
                                                          IM_S32_MIN,
                                                  p_max ? *(ImS32 const*)p_max :
                                                          IM_S32_MAX,
                                                  format, flags);
    case ImGuiDataType_U32:
        return DragBehaviorT<ImU32, ImS32, float>(data_type, (ImU32*)p_v,
                                                  v_speed,
                                                  p_min ? *(ImU32 const*)p_min :
                                                          IM_U32_MIN,
                                                  p_max ? *(ImU32 const*)p_max :
                                                          IM_U32_MAX,
                                                  format, flags);
    case ImGuiDataType_S64:
    ...
    case ImGuiDataType_Float:
        return DragBehaviorT<float, float, float>(data_type, (float*)p_v,
                                                  v_speed,
                                                  p_min ? *(float const*)p_min :
                                                          -FLT_MAX,
                                                  p_max ? *(float const*)p_max :
                                                          FLT_MAX,
                                                  format, flags);
    case ImGuiDataType_Double:
...
bool ImGui::DragScalar(char const* label, ImGuiDataType data_type, void* p_data,
                       float v_speed, void const* p_min, void const* p_max,
                       char const* format, ImGuiSliderFlags flags) {
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems) return false;
    ...
    ImVec2 const label_size = CalcTextSize(label, NULL, true);
    ImRect const frame_bb(window->DC.CursorPos,
                          window->DC.CursorPos +
                              ImVec2(w, label_size.y +
                                            style.FramePadding.y * 2.0f));
    ImRect const total_bb(frame_bb.Min,
    ...
    if (!temp_input_is_active) {
        bool const input_requested_by_tabbing =
        ...
        bool const double_clicked = (hovered && g.IO.MouseClickedCount[0] == 2);
    ...
    // Draw frame
    ImU32 const frame_col = GetColorU32(g.ActiveId == id ?
                                            ImGuiCol_FrameBgActive :
                                        hovered ? ImGuiCol_FrameBgHovered :
                                                  ImGuiCol_FrameBg);
    ...
    // Drag behavior
    bool const value_changed = DragBehavior(id, data_type, p_data, v_speed,
...
bool ImGui::DragScalarN(char const* label, ImGuiDataType data_type,
...
bool ImGui::DragFloat(char const* label, float* v, float v_speed, float v_min,
                      float v_max, char const* format, ImGuiSliderFlags flags) {
    return DragScalar(label, ImGuiDataType_Float, v, v_speed, &v_min, &v_max,
                      format, flags);
}

bool ImGui::DragFloat2(char const* label, float v[2], float v_speed,
...
bool ImGui::DragFloat3(char const* label, float v[3], float v_speed,
                       float v_min, float v_max, char const* format,
                       ImGuiSliderFlags flags) {
...
bool ImGui::DragFloat4(char const* label, float v[4], float v_speed,
                       float v_min, float v_max, char const* format,
                       ImGuiSliderFlags flags) {
...
// this.
bool ImGui::DragFloatRange2(char const* label, float* v_current_min,
                            float* v_current_max, float v_speed, float v_min,
                            float v_max, char const* format,
                            char const* format_max, ImGuiSliderFlags flags) {
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems) return false;
    ...
    bool value_changed = DragScalar("##min", ImGuiDataType_Float, v_current_min,
                                    v_speed, &min_min, &min_max, format,
                                    min_flags);
    PopItemWidth();
    SameLine(0, g.Style.ItemInnerSpacing.x);
...
bool ImGui::DragInt(char const* label, int* v, float v_speed, int v_min,
                    int v_max, char const* format, ImGuiSliderFlags flags) {
    return DragScalar(label, ImGuiDataType_S32, v, v_speed, &v_min, &v_max,
                      format, flags);
}

bool ImGui::DragInt2(char const* label, int v[2], float v_speed, int v_min,
                     int v_max, char const* format, ImGuiSliderFlags flags) {
    return DragScalarN(label, ImGuiDataType_S32, v, 2, v_speed, &v_min, &v_max,
                       format, flags);
}

bool ImGui::DragInt3(char const* label, int v[3], float v_speed, int v_min,
                     int v_max, char const* format, ImGuiSliderFlags flags) {
    return DragScalarN(label, ImGuiDataType_S32, v, 3, v_speed, &v_min, &v_max,
                       format, flags);
}

bool ImGui::DragInt4(char const* label, int v[4], float v_speed, int v_min,
                     int v_max, char const* format, ImGuiSliderFlags flags) {
...
// NB: You likely want to specify the ImGuiSliderFlags_AlwaysClamp when using
// this.
bool ImGui::DragIntRange2(char const* label, int* v_current_min,
                          int* v_current_max, float v_speed, int v_min,
                          int v_max, char const* format, char const* format_max,
                          ImGuiSliderFlags flags) {
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems) return false;
...
template <typename TYPE, typename SIGNEDTYPE, typename FLOATTYPE>
bool ImGui::SliderBehaviorT(ImRect const& bb, ImGuiID id,
                            ImGuiDataType data_type, TYPE* v, const TYPE v_min,
                            const TYPE v_max, char const* format,
    ...
    float const slider_usable_pos_min =
        bb.Min[axis] + grab_padding + grab_sz * 0.5f;
    float const slider_usable_pos_max =
    ...
    if (g.ActiveId == id) {
        ...
        if (g.ActiveIdSource == ImGuiInputSource_Mouse) {
            ...
            if (g.NavActivatePressedId == id && !g.ActiveIdIsJustActivated) {
                                    ...
                                    // when already past the limits
                {
                    ...
                    if (!(flags & ImGuiSliderFlags_NoRoundToFormat))
                        v_new =
                            RoundScalarWithFormatT<TYPE, SIGNEDTYPE>(format,
                                                                     data_type,
                                                                     v_new);
                    float new_clicked_t =
                        ScaleRatioFromValueT<TYPE, SIGNEDTYPE, FLOATTYPE>(
        ...
        if (set_new_value) {
            ...
            // Round to user desired precision based on format string
            if (!(flags & ImGuiSliderFlags_NoRoundToFormat))
                v_new = RoundScalarWithFormatT<TYPE, SIGNEDTYPE>(format,
                                                                 data_type,
                                                                 v_new);
...
bool ImGui::SliderBehavior(ImRect const& bb, ImGuiID id,
    ...
    case ImGuiDataType_S16: {
        ImS32 v32 = (ImS32) * (ImS16*)p_v;
        bool r =
            SliderBehaviorT<ImS32, ImS32, float>(bb, id, ImGuiDataType_S32,
                                                 &v32, *(ImS16 const*)p_min,
                                                 *(ImS16 const*)p_max, format,
                                                 flags, out_grab_bb);
        if (r) *(ImS16*)p_v = (ImS16)v32;
        return r;
    }
    case ImGuiDataType_U16: {
        ImU32 v32 = (ImU32) * (ImU16*)p_v;
        bool r =
            SliderBehaviorT<ImU32, ImS32, float>(bb, id, ImGuiDataType_U32,
                                                 &v32, *(ImU16 const*)p_min,
                                                 *(ImU16 const*)p_max, format,
                                                 flags, out_grab_bb);
        if (r) *(ImU16*)p_v = (ImU16)v32;
        return r;
    }
    case ImGuiDataType_S32:
        IM_ASSERT(*(ImS32 const*)p_min >= IM_S32_MIN / 2 &&
                  *(ImS32 const*)p_max <= IM_S32_MAX / 2);
    ...
    case ImGuiDataType_U32:
        IM_ASSERT(*(ImU32 const*)p_max <= IM_U32_MAX / 2);
        return SliderBehaviorT<ImU32, ImS32, float>(bb, id, data_type,
                                                    ...
                                                    *(ImU32 const*)p_max,
                                                    format, flags, out_grab_bb);
    case ImGuiDataType_S64:
        IM_ASSERT(*(ImS64 const*)p_min >= IM_S64_MIN / 2 &&
                  *(ImS64 const*)p_max <= IM_S64_MAX / 2);
        return SliderBehaviorT<ImS64, ImS64, double>(bb, id, data_type,
                                                     ...
                                                     *(ImS64 const*)p_max,
                                                     ...
                                                     out_grab_bb);
    case ImGuiDataType_U64:
        IM_ASSERT(*(ImU64 const*)p_max <= IM_U64_MAX / 2);
        return SliderBehaviorT<ImU64, ImS64, double>(bb, id, data_type,
                                                     ...
                                                     *(ImU64 const*)p_max,
                                                     ...
                                                     out_grab_bb);
    case ImGuiDataType_Float:
        IM_ASSERT(*(float const*)p_min >= -FLT_MAX / 2.0f &&
                  *(float const*)p_max <= FLT_MAX / 2.0f);
        return SliderBehaviorT<float, float, float>(bb, id, data_type,
                                                    ...
                                                    *(float const*)p_max,
                                                    format, flags, out_grab_bb);
    case ImGuiDataType_Double:
        IM_ASSERT(*(double const*)p_min >= -DBL_MAX / 2.0f &&
                  *(double const*)p_max <= DBL_MAX / 2.0f);
...
bool ImGui::SliderScalar(char const* label, ImGuiDataType data_type,
                         void* p_data, void const* p_min, void const* p_max,
                         char const* format, ImGuiSliderFlags flags) {
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems) return false;
    ...
    if (temp_input_is_active) {
        // Only clamp CTRL+Click input when ImGuiSliderFlags_AlwaysClamp is set
        bool const is_clamp_input = (flags & ImGuiSliderFlags_AlwaysClamp) != 0;
        return TempInputScalar(frame_bb, id, label, data_type, p_data, format,
                               is_clamp_input ? p_min : NULL,
                               is_clamp_input ? p_max : NULL);
    }
    ...
    // Slider behavior
    ImRect grab_bb;
    bool const value_changed = SliderBehavior(frame_bb, id, data_type, p_data,
                                              p_min, p_max, format, flags,
                                              &grab_bb);
    if (value_changed) MarkItemEdited(id);
...
bool ImGui::SliderScalarN(char const* label, ImGuiDataType data_type, void* v,
                          int components, void const* v_min, void const* v_max,
                          char const* format, ImGuiSliderFlags flags) {
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems) return false;
...
bool ImGui::SliderFloat(char const* label, float* v, float v_min, float v_max,
                        char const* format, ImGuiSliderFlags flags) {
    return SliderScalar(label, ImGuiDataType_Float, v, &v_min, &v_max, format,
                        flags);
}

bool ImGui::SliderFloat2(char const* label, float v[2], float v_min,
                         float v_max, char const* format,
                         ImGuiSliderFlags flags) {
...
bool ImGui::SliderFloat3(char const* label, float v[3], float v_min,
                         float v_max, char const* format,
                         ImGuiSliderFlags flags) {
...
bool ImGui::SliderFloat4(char const* label, float v[4], float v_min,
                         float v_max, char const* format,
                         ImGuiSliderFlags flags) {
...
bool ImGui::SliderAngle(char const* label, float* v_rad, float v_degrees_min,
                        float v_degrees_max, char const* format,
                        ImGuiSliderFlags flags) {
...
bool ImGui::SliderInt(char const* label, int* v, int v_min, int v_max,
                      char const* format, ImGuiSliderFlags flags) {
    return SliderScalar(label, ImGuiDataType_S32, v, &v_min, &v_max, format,
                        flags);
}

bool ImGui::SliderInt2(char const* label, int v[2], int v_min, int v_max,
                       char const* format, ImGuiSliderFlags flags) {
    return SliderScalarN(label, ImGuiDataType_S32, v, 2, &v_min, &v_max, format,
                         flags);
}

bool ImGui::SliderInt3(char const* label, int v[3], int v_min, int v_max,
                       char const* format, ImGuiSliderFlags flags) {
    return SliderScalarN(label, ImGuiDataType_S32, v, 3, &v_min, &v_max, format,
                         flags);
}

bool ImGui::SliderInt4(char const* label, int v[4], int v_min, int v_max,
                       char const* format, ImGuiSliderFlags flags) {
    return SliderScalarN(label, ImGuiDataType_S32, v, 4, &v_min, &v_max, format,
                         flags);
}

bool ImGui::VSliderScalar(char const* label, ImVec2 const& size,
    ...
    ImVec2 const label_size = CalcTextSize(label, NULL, true);
    ImRect const frame_bb(window->DC.CursorPos, window->DC.CursorPos + size);
    ImRect const bb(frame_bb.Min,
                    frame_bb.Max +
...
bool ImGui::VSliderFloat(char const* label, ImVec2 const& size, float* v,
                         float v_min, float v_max, char const* format,
                         ImGuiSliderFlags flags) {
...
bool ImGui::VSliderInt(char const* label, ImVec2 const& size, int* v, int v_min,
                       int v_max, char const* format, ImGuiSliderFlags flags) {
...
char const* ImParseFormatFindEnd(char const* fmt) {
    ...
    if (fmt[0] != '%') return fmt;
    unsigned int const ignored_uppercase_mask = (1 << ('I' - 'A')) |
                                                (1 << ('L' - 'A'));
    unsigned int const ignored_lowercase_mask =
        (1 << ('h' - 'a')) | (1 << ('j' - 'a')) | (1 << ('l' - 'a')) |
        (1 << ('t' - 'a')) | (1 << ('w' - 'a')) | (1 << ('z' - 'a'));
...
char const* ImParseFormatTrimDecorations(char const* fmt, char* buf,
                                         size_t buf_size) {
...
bool ImGui::TempInputText(ImRect const& bb, ImGuiID id, char const* label,
    ...
    ImGuiContext& g = *GImGui;
    bool const init = (g.TempInputId != id);
...
bool ImGui::TempInputScalar(ImRect const& bb, ImGuiID id, char const* label,
                            ImGuiDataType data_type, void* p_data,
                            char const* format, void const* p_clamp_min,
                            void const* p_clamp_max) {
    char fmt_buf[32];
    char data_buf[32];
    format =
...
bool ImGui::InputScalar(char const* label, ImGuiDataType data_type,
                        void* p_data, void const* p_step,
                        void const* p_step_fast, char const* format,
                        ImGuiInputTextFlags flags) {
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems) return false;
    ...
    if (p_step != NULL) {
        ...
        // Step buttons
        ImVec2 const backup_frame_padding = style.FramePadding;
...
bool ImGui::InputScalarN(char const* label, ImGuiDataType data_type,
                         void* p_data, int components, void const* p_step,
                         void const* p_step_fast, char const* format,
                         ImGuiInputTextFlags flags) {
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems) return false;
...
bool ImGui::InputFloat(char const* label, float* v, float step, float step_fast,
...
bool ImGui::InputFloat2(char const* label, float v[2], char const* format,
                        ImGuiInputTextFlags flags) {
    return InputScalarN(label, ImGuiDataType_Float, v, 2, NULL, NULL, format,
                        flags);
}

bool ImGui::InputFloat3(char const* label, float v[3], char const* format,
                        ImGuiInputTextFlags flags) {
    return InputScalarN(label, ImGuiDataType_Float, v, 3, NULL, NULL, format,
                        flags);
}

bool ImGui::InputFloat4(char const* label, float v[4], char const* format,
                        ImGuiInputTextFlags flags) {
    return InputScalarN(label, ImGuiDataType_Float, v, 4, NULL, NULL, format,
                        flags);
}

bool ImGui::InputInt(char const* label, int* v, int step, int step_fast,
                     ImGuiInputTextFlags flags) {
...
bool ImGui::InputDouble(char const* label, double* v, double step,
                        double step_fast, char const* format,
                        ImGuiInputTextFlags flags) {
    flags |= ImGuiInputTextFlags_CharsScientific;
...
bool ImGui::InputText(char const* label, char* buf, size_t buf_size,
                      ImGuiInputTextFlags flags,
...
bool ImGui::InputTextMultiline(char const* label, char* buf, size_t buf_size,
...
bool ImGui::InputTextWithHint(char const* label, char const* hint, char* buf,
...
static ImVec2 InputTextCalcTextSizeW(ImWchar const* text_begin,
    ...
    if (out_offset)
        *out_offset =
            ImVec2(line_width,
                   text_size.y +
                       line_height); // offset allow for the possibility of
...
static int STB_TEXTEDIT_STRINGLEN(ImGuiInputTextState const* obj) {
    return obj->CurLenW;
}
static ImWchar STB_TEXTEDIT_GETCHAR(ImGuiInputTextState const* obj, int idx) {
...
static void STB_TEXTEDIT_LAYOUTROW(StbTexteditRow* r, ImGuiInputTextState* obj,
                                   int line_start_idx) {
    ImWchar const* text = obj->TextW.Data;
    ImWchar const* text_remaining = NULL;
    ImVec2 const size = InputTextCalcTextSizeW(text + line_start_idx,
                                               text + obj->CurLenW,
...
static void STB_TEXTEDIT_DELETECHARS(ImGuiInputTextState* obj, int pos, int n) {
    ...
    ImWchar const* src = obj->TextW.Data + pos + n;
    while (ImWchar c = *src++)
        *dst++ = c;
    *dst = '\0';
}
...
void ImGuiInputTextCallbackData::InsertChars(int pos, char const* new_text,
                                             char const* new_text_end) {
    bool const is_resizable = (Flags & ImGuiInputTextFlags_CallbackResize) != 0;
    int const new_text_len = new_text_end ? (int)(new_text_end - new_text) :
                                            (int)strlen(new_text);
...
static bool InputTextFilterCharacter(unsigned int* p_char,
                                         ...
                                         ImGuiInputTextFlags_CharsScientific)))
    {
        ...
        ImGuiContext& g = *GImGui;
        unsigned const c_decimal_point =
...
bool ImGui::InputTextEx(char const* label, char const* hint, char* buf,
                        int buf_size, ImVec2 const& size_arg,
                        ImGuiInputTextFlags flags,
                        ImGuiInputTextCallback callback,
    ...
    ImRect const total_bb(frame_bb.Min, frame_bb.Min + total_size);
    ...
    bool const init_changed_specs =
        (state != NULL && state->Stb.single_line != !is_multiline);
    bool const init_make_active = (user_clicked || user_scroll_finish ||
    ...
    if ((init_state && g.ActiveId != id) || init_changed_specs) {
        ...
        bool const recycle_state = (state->ID == id && !init_changed_specs);
        if (recycle_state) {
    ...
    if (g.ActiveId != id && init_make_active) {
        ...
        if (is_multiline || (flags & ImGuiInputTextFlags_CallbackHistory))
            g.ActiveIdUsingNavDirMask |= (1 << ImGuiDir_Up) |
                                         (1 << ImGuiDir_Down);
    ...
    // cursor or selection
    bool const render_cursor = (g.ActiveId == id) ||
    ...
    if (g.ActiveId == id) {
        ...
        if (select_all) {
            ...
            int const multiclick_count = (io.MouseClickedCount[0] - 2);
        ...
        // SDL.
        bool const ignore_char_inputs = (io.KeyCtrl && !io.KeyAlt) ||
                                        (is_osx && io.KeySuper);
    ...
    if (g.ActiveId == id && !g.ActiveIdIsJustActivated && !clear_active_id) {
        ...
        bool const is_paste =
            ...
            !is_readonly;
        bool const is_undo = ((is_shortcut_key && IsKeyPressed(ImGuiKey_Z)) &&
        ...
        if (apply_edit_back_to_user_buffer) {
                          ...
                          ImGuiInputTextFlags_CallbackAlways)) != 0)
            {
                ...
                if (event_flag) {
                    ...
                    int const utf8_cursor_pos = callback_data.CursorPos =
                        ImTextCountUtf8BytesFromStr(text,
                                                    text + state->Stb.cursor);
                    int const utf8_selection_start =
                        callback_data.SelectionStart =
                            ImTextCountUtf8BytesFromStr(text,
                                                        text +
                                                            state->Stb
                                                                .select_start);
                    int const utf8_selection_end = callback_data.SelectionEnd =
                        ImTextCountUtf8BytesFromStr(text,
                                                    text +
    ...
    ImVec4 const clip_rect(frame_bb.Min.x, frame_bb.Min.y,
    ...
    char const* buf_display = buf_display_from_state ? state->TextA.Data :
                                                       buf; //-V595
    char const* buf_display_end =
    ...
    if (render_cursor || render_selection) {
        ...
        ImWchar const* text_begin = state->TextW.Data;
        ImVec2 cursor_offset, select_start_offset;

        {
            // Find lines numbers straddling 'cursor' (slot 0) and
            // 'select_start' (slot 1) positions.
            ImWchar const* searches_input_ptr[2] = { NULL, NULL };
            ...
            for (ImWchar const* s = text_begin; *s != 0; s++)
                if (*s == '\n') {
                    line_count++;
        ...
        if (render_cursor && state->CursorFollow) {
            ...
            if (!(flags & ImGuiInputTextFlags_NoHorizontalScroll)) {
                ...
                float const visible_width = inner_size.x - style.FramePadding.x;
        ...
        // Draw selection
        ImVec2 const draw_scroll = ImVec2(state->ScrollX, 0.0f);
        if (render_selection) {
            ImWchar const* text_selected_begin =
                text_begin +
                ImMin(state->Stb.select_start, state->Stb.select_end);
            ImWchar const* text_selected_end =
                text_begin +
                ImMax(state->Stb.select_start, state->Stb.select_end);
                ...
                GetColorU32(ImGuiCol_TextSelectedBg,
                            render_cursor ?
                                ...
                                0.6f); // FIXME: current code flow mandate that
                                       // render_cursor is always true here, we
                                       // are leaving the transparent one for
                                       // tests.
            float bg_offy_up =
                is_multiline ?
                    0.0f :
            ...
            for (ImWchar const* p = text_selected_begin; p < text_selected_end;)
            {
...
bool ImGui::ColorEdit3(char const* label, float col[3],
...
// resetting.
static void ColorEditRestoreHS(float const* col, float* H, float* S, float* V) {
...
bool ImGui::ColorEdit4(char const* label, float col[4],
                       ImGuiColorEditFlags flags) {
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems) return false;
    ...
    float const w_button = (flags & ImGuiColorEditFlags_NoSmallPreview) ?
                               0.0f :
    ...
    // conversions
    ImGuiColorEditFlags const flags_untouched = flags;
        ...
        (flags & ImGuiColorEditFlags_NoInputs) == 0)
    {
        ...
        int const fmt_idx = hide_prefix                              ? 0 :
    ...
    if (!(flags & ImGuiColorEditFlags_NoSmallPreview)) {
        ...
        ImVec4 const col_v4(col[0], col[1], col[2], alpha ? col[3] : 1.0f);
    ...
    if (label != label_display_end && !(flags & ImGuiColorEditFlags_NoLabel)) {
        float const text_offset_x = (flags & ImGuiColorEditFlags_NoInputs) ?
                                        w_button :
...
bool ImGui::ColorPicker3(char const* label, float col[3],
                         ImGuiColorEditFlags flags) {
...
bool ImGui::ColorPicker4(char const* label, float col[4],
                         ImGuiColorEditFlags flags, float const* ref_col) {
    ImGuiContext& g = *GImGui;
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems) return false;
    ...
    float sv_picker_size =
        ImMax(bars_width * 1,
              width -
                  (alpha_bar ? 2 : 1) *
                      (bars_width + style.ItemInnerSpacing
    ...
    if (value_changed) {
        if (flags & ImGuiColorEditFlags_InputRGB) {
            ...
            ColorEditRestoreHS(col, &H, &S,
                               &V); // Fix local Hue as display below will use
                                    // it immediately.
        }
        else if (flags & ImGuiColorEditFlags_InputHSV) {
            H = col[0];
    ...
    ImU32 const col_black = IM_COL32(0, 0, 0, style_alpha8);
    ImU32 const col_white = IM_COL32(255, 255, 255, style_alpha8);
    ImU32 const col_midgrey = IM_COL32(128, 128, 128, style_alpha8);
    ImU32 const col_hues[6 + 1] = {
    ...
    if (flags & ImGuiColorEditFlags_PickerHueWheel) {
        // Render Hue Wheel
        float const aeps = 0.5f / wheel_r_outer; // Half a pixel arc length in
...
bool ImGui::ColorButton(char const* desc_id, ImVec4 const& col,
                        ImGuiColorEditFlags flags, ImVec2 size) {
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems) return false;

    ImGuiContext& g = *GImGui;
    ImGuiID const id = window->GetID(desc_id);
...
void ImGui::ColorTooltip(char const* text, float const* col,
                         ImGuiColorEditFlags flags) {
    ImGuiContext& g = *GImGui;
...
void ImGui::ColorPickerOptionsPopup(float const* ref_col,
                                    ImGuiColorEditFlags flags) {
...
bool ImGui::TreeNode(void const* ptr_id, char const* fmt, ...) {
    va_list args;
    va_start(args, fmt);
...
bool ImGui::TreeNodeV(void const* ptr_id, char const* fmt, va_list args) {
...
bool ImGui::TreeNodeEx(char const* str_id, ImGuiTreeNodeFlags flags,
                       char const* fmt, ...) {
    va_list args;
    va_start(args, fmt);
...
bool ImGui::TreeNodeEx(void const* ptr_id, ImGuiTreeNodeFlags flags,
                       char const* fmt, ...) {
    va_list args;
    va_start(args, fmt);
...
bool ImGui::TreeNodeExV(char const* str_id, ImGuiTreeNodeFlags flags,
...
bool ImGui::TreeNodeExV(void const* ptr_id, ImGuiTreeNodeFlags flags,
                        char const* fmt, va_list args) {
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems) return false;
...
bool ImGui::TreeNodeBehavior(ImGuiID id, ImGuiTreeNodeFlags flags,
                             char const* label, char const* label_end) {
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems) return false;
    ...
    ImVec2 const label_size = CalcTextSize(label, label_end, false);

    // We vertically grow up to current line height up the typical widget
    // height.
    float const frame_height =
        ImMax(ImMin(window->DC.CurrLineSize.y,
    ...
    float const arrow_hit_x1 =
        (text_pos.x - text_offset_x) - style.TouchExtraPadding.x;
    float const arrow_hit_x2 = (text_pos.x - text_offset_x) +
                               (g.FontSize + padding.x * 2.0f) +
                               style.TouchExtraPadding.x;
    bool const is_mouse_x_over_arrow =
    ...
    if (display_frame) {
        // Framed type
        ImU32 const bg_col = GetColorU32((held && hovered) ?
...
void ImGui::TreePush(void const* ptr_id) {
    ImGuiWindow* window = GetCurrentWindow();
    Indent();
    window->DC.TreeDepth++;
    PushID(ptr_id);
...
bool ImGui::CollapsingHeader(char const* label, bool* p_visible,
...
bool ImGui::Selectable(char const* label, bool selected,
                       ImGuiSelectableFlags flags, ImVec2 const& size_arg) {
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems) return false;
    ...
    // other widgets.
    bool const span_all_columns =
        (flags & ImGuiSelectableFlags_SpanAllColumns) != 0;
    float const min_x = span_all_columns ? window->ParentWorkRect.Min.x : pos.x;
    float const max_x = span_all_columns ? window->ParentWorkRect.Max.x :
    ...
    // on both sides
    ImVec2 const text_min = pos;
    ImVec2 const text_max(min_x + size.x, pos.y + size.y);
    ...
    if ((flags & ImGuiSelectableFlags_NoPadWithHalfSpacing) == 0) {
        float const spacing_x = span_all_columns ? 0.0f : style.ItemSpacing.x;
    ...
    float const backup_clip_rect_min_x = window->ClipRect.Min.x;
    float const backup_clip_rect_max_x = window->ClipRect.Max.x;
    if (span_all_columns) {
        window->ClipRect.Min.x = window->ParentWorkRect.Min.x;
        window->ClipRect.Max.x = window->ParentWorkRect.Max.x;
    }

    bool const disabled_item = (flags & ImGuiSelectableFlags_Disabled) != 0;
    bool const item_add =
        ItemAdd(bb, id, NULL,
    ...
    if (held && (flags & ImGuiSelectableFlags_DrawHoveredWhenHeld))
        hovered = true;
    if (hovered || selected) {
        ImU32 const col = GetColorU32((held && hovered) ?
                                          ImGuiCol_HeaderActive :
                                      hovered ? ImGuiCol_HeaderHovered :
                                                ImGuiCol_Header);
...
bool ImGui::Selectable(char const* label, bool* p_selected,
                       ImGuiSelectableFlags flags, ImVec2 const& size_arg) {
...
// item_height).
bool ImGui::BeginListBox(char const* label, ImVec2 const& size_arg) {
    ImGuiContext& g = *GImGui;
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems) return false;
...
bool ImGui::ListBoxHeader(char const* label, int items_count,
                          int height_in_items) {
...
bool ImGui::ListBox(char const* label, int* current_item,
                    char const* const items[], int items_count,
                    int height_items) {
    bool const value_changed = ListBox(label, current_item, Items_ArrayGetter,
...
int ImGui::PlotEx(ImGuiPlotType plot_type, char const* label,
    ...
    ImRect const frame_bb(window->DC.CursorPos,
                          window->DC.CursorPos + frame_size);
    ImRect const inner_bb(frame_bb.Min + style.FramePadding,
    ...
    if (values_count >= values_count_min) {
        ...
        if (hovered && inner_bb.Contains(g.IO.MousePos)) {
            float const t = ImClamp((g.IO.MousePos.x - inner_bb.Min.x) /
        ...
        ImU32 const col_base = GetColorU32((plot_type == ImGuiPlotType_Lines) ?
                                               ImGuiCol_PlotLines :
                                               ImGuiCol_PlotHistogram);
        ImU32 const col_hovered = GetColorU32(
        ...
        for (int n = 0; n < res_w; n++) {
            ...
            float const v1 = values_getter(data, (v1_idx + values_offset + 1) %
                                                     values_count);
            ImVec2 const tp1 =
...
struct ImGuiPlotArrayGetterData {
    ...
    ImGuiPlotArrayGetterData(float const* values, int stride) {
        Values = values;
        Stride = stride;
    }
};
...
static float Plot_ArrayGetter(void* data, int idx) {
    ...
    float const v =
        *(float const*)(void const*)((unsigned char const*)plot_data->Values +
                                     (size_t)idx * plot_data->Stride);
    return v;
}

void ImGui::PlotLines(char const* label, float const* values, int values_count,
                      int values_offset, char const* overlay_text,
...
void ImGui::PlotLines(char const* label,
                      float (*values_getter)(void* data, int idx), void* data,
                      int values_count, int values_offset,
                      char const* overlay_text, float scale_min,
...
void ImGui::PlotHistogram(char const* label, float const* values,
                          int values_count, int values_offset,
                          char const* overlay_text, float scale_min,
...
void ImGui::PlotHistogram(char const* label,
...
void ImGui::Value(char const* prefix, int v) { Text("%s: %d", prefix, v); }
...
void ImGui::EndMenuBar() {
        ...
        (g.NavWindow->Flags & ImGuiWindowFlags_ChildMenu))
    {
            ...
            (g.NavMoveFlags & ImGuiNavMoveFlags_Forwarded) == 0)
        {
            ...
            // worth bothering)
            ImGuiNavLayer const layer = ImGuiNavLayer_Menu;
...
bool ImGui::BeginViewportSideBar(char const* name, ImGuiViewport* viewport_p,
...
static bool IsRootOfOpenMenuSet() {
    ...
    // each others (e.g. inside menu bar vs loose menu items) based on parent
    // ID. This would however prevent the use of e.g. PuhsID() user code
    // submitting menus. Previously this worked between popup and a first child
    // menu because the first child menu always had the _ChildWindow flag,
    // making  hovering on parent popup possible while first child menu was
    // focused - but this was generally a bug with other side effects. Instead
    // we don't treat Popup specifically (in order to consistently support menu
    // features in them), maybe the first child menu of a Popup doesn't have the
    // _ChildWindow flag, and we rely on this IsRootOfOpenMenuSet() check to
    // allow hovering between root window/popup and first chilld menu.
    ImGuiPopupData const* upper_popup =
...
bool ImGui::BeginMenuEx(char const* label, char const* icon, bool enabled) {
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems) return false;
    ...
    // get accross (otherwise e.g. resizing borders with
    ...
    bool const menuset_is_open = IsRootOfOpenMenuSet();
    ...
    ImGuiMenuColumns const* offsets = &window->DC.MenuColumns;
    bool pressed;
    ImGuiSelectableFlags const selectable_flags =
    ...
    if (window->DC.LayoutType == ImGuiLayoutType_Horizontal) {
        ...
        ImVec2 text_pos(window->DC.CursorPos.x + offsets->OffsetLabel,
                        window->DC.CursorPos.y +
                            window->DC.CurrLineTextBaseOffset);
        pressed =
            Selectable("", menu_is_open, selectable_flags, ImVec2(w, 0.0f));
        ...
        float min_w = window->DC.MenuColumns
                          .DeclColumns(icon_w, label_size.x, 0.0f,
        ...
        RenderArrow(window->DrawList,
                    pos + ImVec2(offsets->OffsetMark + extra_w +
                                     g.FontSize * 0.30f,
                                 0.0f),
    ...
    bool const hovered = (g.HoveredId == id) && enabled;
    ...
    if (menu_is_open) {
        SetNextWindowPos(popup_pos,
                         ImGuiCond_Always); // Note: this is super misleading!
                                            // The value will serve as reference
                                            // for FindBestWindowPosForPopup(),
                                            // not actual pos.
        PushStyleVar(ImGuiStyleVar_ChildRounding,
                     style.PopupRounding); // First level will use
                                           // _PopupRounding, subsequent will
                                           // use _ChildRounding
        menu_is_open =
            BeginPopupEx(id, flags); // menu_is_open can be 'false' when the
...
bool ImGui::MenuItemEx(char const* label, char const* icon,
                       char const* shortcut, bool selected, bool enabled) {
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems) return false;
    ...
    if (window->DC.LayoutType == ImGuiLayoutType_Horizontal) {
        ...
        float checkmark_w = IM_FLOOR(g.FontSize * 1.20f);
        float min_w = window->DC.MenuColumns
                          .DeclColumns(icon_w, label_size.x, shortcut_w,
...
bool ImGui::MenuItem(char const* label, char const* shortcut, bool selected,
                     bool enabled) {
    return MenuItemEx(label, NULL, shortcut, selected, enabled);
}

bool ImGui::MenuItem(char const* label, char const* shortcut, bool* p_selected,
                     bool enabled) {
...
static inline int TabItemGetSectionIdx(ImGuiTabItem const* tab) {
    return (tab->Flags & ImGuiTabItemFlags_Leading)  ? 0 :
           (tab->Flags & ImGuiTabItemFlags_Trailing) ? 2 :
                                                       1;
}

static int IMGUI_CDECL TabItemComparerBySection(void const* lhs,
                                                void const* rhs) {
    ImGuiTabItem const* a = (ImGuiTabItem const*)lhs;
    ImGuiTabItem const* b = (ImGuiTabItem const*)rhs;
    int const a_section = TabItemGetSectionIdx(a);
    int const b_section = TabItemGetSectionIdx(b);
    if (a_section != b_section) return a_section - b_section;
    return (int)(a->IndexDuringLayout - b->IndexDuringLayout);
}

static int IMGUI_CDECL TabItemComparerByBeginOrder(void const* lhs,
                                                   void const* rhs) {
    ImGuiTabItem const* a = (ImGuiTabItem const*)lhs;
    ImGuiTabItem const* b = (ImGuiTabItem const*)rhs;
...
static ImGuiTabBar* GetTabBarFromTabBarRef(ImGuiPtrOrIndex const& ref) {
    ImGuiContext& g = *GImGui;
...
bool ImGui::BeginTabBar(char const* str_id, ImGuiTabBarFlags flags) {
    ...
    return BeginTabBarEx(tab_bar, tab_bar_bb,
                         flags | ImGuiTabBarFlags_IsFocused, NULL);
}

bool ImGui::BeginTabBarEx(ImGuiTabBar* tab_bar, ImRect const& tab_bar_bb,
    ...
    //    ImGuiCol_TabActive : ImGuiCol_TabUnfocusedActive);
    ImU32 const col = GetColorU32(ImGuiCol_Border);
    float const y = tab_bar->BarRect.Max.y - 1.0f;
    if (dock_node != NULL) {
        float const separator_min_x =
            dock_node->Pos.x + window->WindowBorderSize;
        float const separator_max_x =
...
static void ImGui::TabBarLayout(ImGuiTabBar* tab_bar) {
    ...
    // width!)
    bool const tab_list_popup_button =
...
void ImGui::TabBarQueueReorder(ImGuiTabBar* tab_bar, ImGuiTabItem const* tab,
                               int offset) {
    IM_ASSERT(offset != 0);
...
void ImGui::TabBarQueueReorderFromMousePos(ImGuiTabBar* tab_bar,
                                           ImGuiTabItem const* src_tab,
                                           ImVec2 mouse_pos) {
    ImGuiContext& g = *GImGui;
    IM_ASSERT(tab_bar->ReorderRequestTabId == 0);
    if ((tab_bar->Flags & ImGuiTabBarFlags_Reorderable) == 0) return;

    bool const is_central_section =
        (src_tab->Flags & ImGuiTabItemFlags_SectionMask_) == 0;
    float const bar_offset =
        tab_bar->BarRect.Min.x -
        (is_central_section ? tab_bar->ScrollingTarget : 0);

    // Count number of contiguous tabs we are crossing over
    int const dir = (bar_offset + src_tab->Offset) > mouse_pos.x ? -1 : +1;
    int const src_idx = tab_bar->Tabs.index_from_ptr(src_tab);
    int dst_idx = src_idx;
    for (int i = src_idx; i >= 0 && i < tab_bar->Tabs.Size; i += dir) {
        // Reordered tabs must share the same section
        ImGuiTabItem const* dst_tab = &tab_bar->Tabs[i];
        ...
        float const x2 = bar_offset + dst_tab->Offset + dst_tab->Width +
                         g.Style.ItemInnerSpacing.x;
...
bool ImGui::TabBarProcessReorder(ImGuiTabBar* tab_bar) {
    ...
    ImGuiTabItem* src_tab = (tab_bar->ReorderRequestOffset > 0) ? tab1 + 1 :
                                                                  tab2;
    ImGuiTabItem* dst_tab = (tab_bar->ReorderRequestOffset > 0) ? tab1 :
                                                                  tab2 + 1;
    int const move_count = (tab_bar->ReorderRequestOffset > 0) ?
...
static ImGuiTabItem* ImGui::TabBarTabListPopupButton(ImGuiTabBar* tab_bar) {
    ...
    float const tab_list_popup_button_width =
...
bool ImGui::BeginTabItem(char const* label, bool* p_open,
...
bool ImGui::TabItemButton(char const* label, ImGuiTabItemFlags flags) {
    ImGuiContext& g = *GImGui;
    ImGuiWindow* window = g.CurrentWindow;
    if (window->SkipItems) return false;
...
bool ImGui::TabItemEx(ImGuiTabBar* tab_bar, char const* label, bool* p_open,
    ...
    ImGuiStyle const& style = g.Style;
    ImGuiID const id = TabBarCalcTabID(tab_bar, label, docked_window);
    ...
    ImVec2 const backup_main_cursor_pos = window->DC.CursorPos;
    ...
    bool const want_clip_rect = is_central_section &&
    ...
    bool const single_floating_window_node = node && node->IsFloatingNode() &&
                                             (node->Windows.Size == 1);
...
#if !POPPY_CUSTOM
    ImU32 const tab_col = GetColorU32(
...
ImVec2 ImGui::TabItemCalcSize(char const* label, bool has_close_button) {
    ...
    if (has_close_button)
        size.x += 2 * g.Style.FramePadding.x +
                  (g.Style.ItemInnerSpacing.x +
...
void ImGui::TabItemBackground(ImDrawList* draw_list, ImRect const& bb,
    ...
    bool const is_dockspace_tab = flags & ImGuiTabItemFlags_DockSpaceTab;
    ...
    float const y1 = bb.Min.y + (is_dockspace_tab ? 0.0f : 1.0f);
    float const y2 =
...
void ImGui::TabItemLabelAndCloseButton(ImDrawList* draw_list, ImRect const& bb,
                                       ImGuiTabItemFlags flags,
                                       ImVec2 frame_padding, char const* label,
    ...
    ImVec2 const button_pos(ImMax(bb.Min.x, bb.Max.x - frame_padding.x * 2.0f -
                                                button_sz),
                            bb.Min.y);
    ...
    bool unsaved_marker_visible = (flags & ImGuiTabItemFlags_UnsavedDocument) !=
                                      0 &&
    ...
    if (close_button_visible) {
        ...
        ImRect const bullet_bb(button_pos, button_pos +
    ...
    if (close_button_visible || unsaved_marker_visible) {
        text_pixel_clip_bb.Max.x -= close_button_visible ? (button_sz) :
                                                           (button_sz * 0.80f);
