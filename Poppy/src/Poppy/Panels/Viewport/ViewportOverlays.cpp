...
namespace poppy {
	...
	void ViewportOverlays::display() {
		drawLightOverlay<PointLightComponent>();
		drawLightOverlay<SpotLightComponent>();
		drawLightOverlay<DirectionalLightComponent>();
		drawLightOverlay<SkyLightComponent>();
	}
	
	template <typename Light>
	void ViewportOverlays::drawLightOverlay() {
		auto* const scene = viewport->scene();
		auto* const selection = viewport->selection();
		
		auto view = scene->view<TransformMatrixComponent const, Light const>();
		view.each([&](auto const entity, TransformMatrixComponent const& transform, Light const& light) {
			...
			drawOneLightOverlay(viewport->scene()->getHandle(entity),
								positionInWindow,
	...
	void ViewportOverlays::drawSpotlightVizWS(bloom::EntityID entity,
