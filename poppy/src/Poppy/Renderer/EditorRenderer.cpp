...
namespace poppy {
	...
	std::unique_ptr<bloom::Framebuffer> EditorRenderer::createFramebuffer(mtl::int2 size) const {
		return mRenderer->createFramebuffer(size);
	}
	
	std::unique_ptr<bloom::Framebuffer> EditorRenderer::createDebugFramebuffer(mtl::int2 size) const {
		return mRenderer->createDebugFramebuffer(size);
	}
	
	std::unique_ptr<EditorFramebuffer> EditorRenderer::createEditorFramebuffer(mtl::int2 size) const {
	...
	/// MARK: Initialization
	EditorRenderer::EditorRenderer(bloom::Reciever reciever, std::shared_ptr<Renderer> renderer):
	...
	void EditorRenderer::submit(SkyLight const& light) {
		mRenderer->submit(light);
	}
	...
	/// MARK: Draw
	void EditorRenderer::draw(Framebuffer& framebuffer, CommandQueue& commandQueue) {
	...
	void EditorRenderer::selectedObjectsPass(EditorFramebuffer& framebuffer, bloom::CommandQueue& commandQueue) {
		std::sort(selectedObjects.begin(), selectedObjects.end(), selectedObjectsOrder);
		
		/* upload selected Transforms */ {
			std::size_t const targetBufferSize = selectedObjects.size() * sizeof(float4x4);
			...
			device().fillManagedBuffer(selectedTransformsBuffer, selectedObjects.data().transform, targetBufferSize);
		}
										 ...
										 OverlayDrawDescription const& drawDesc)
	{
		/* upload Editor Draw Data */ {
			...
			device().fillManagedBuffer(editorDrawDataBuffer, &drawData, sizeof drawData);
		}
		...
		auto& renderer = utl::down_cast<ForwardRenderer&>(*mRenderer);
		auto& fwFramebuffer = utl::down_cast<ForwardRendererFramebuffer&>(framebuffer);
