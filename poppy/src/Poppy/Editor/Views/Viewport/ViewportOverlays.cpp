...
namespace poppy {
	...
	static void drawArrow(mtl::float2 from, mtl::float2 to, float headSize, mtl::float4 color, float thickness) {
		float2 const p0 = from;
		float2 const line = to - from;
		
		float const angle = std::acos(mtl::dot(mtl::normalize(line), float2(1, 0)));
		float2x2 const arrowRot = {
	...
	void ViewportOverlays::init(Viewport* viewport) {
	...
	bloom::EntityHandle ViewportOverlays::hitTest(mtl::float2 positionInView) {
		...
		if (auto result = hitTestLightOverlays<DirectionalLightComponent>(positionInView)) {
			return result;
		}
	...
	void ViewportOverlays::drawLightOverlays() {
		...
		for (auto&& scene: viewport->editor().coreSystems().sceneSystem().scenes()) {
			...
			view.each([&](auto const id, TransformMatrixComponent const& transform, Light const& light) {
				auto const positionVS = viewport->worldSpaceToViewSpace(transform.matrix.column(3).xyz);
				if (positionVS.z < 0 || positionVS.z > 1) {
					return;
				}
				auto const positionInWindow = viewport->viewSpaceToWindowSpace(positionVS.xy);
	...
	{
		drawPointLightIcon(positionInWindow,
						   light.common.color);
	}
											   ...
											   DirectionalLight const& light)
	{
		...
		drawDirectionalLightIcon(positionInWindow,
								 lightDirWS,
								 light.common.color);
	}
	...
	{
		drawSkyLightIcon(positionInWindow,
						 light.common.color);
	}
	
	void ViewportOverlays::drawPointLightIcon(mtl::float2 position, mtl::float3 color) {
		float const size = 20;
		auto* const drawList = ImGui::GetWindowDrawList();
	...
	void ViewportOverlays::drawSpotLightIcon(mtl::float2 position, mtl::float3 color) {
	...
	void ViewportOverlays::drawSkyLightIcon(mtl::float2 position, mtl::float3 color) {
		float const size = 20;
		...
		auto doDraw = [&](float4 color, float thickness) {
			...
			ImGui::GetWindowDrawList()->AddPolyline(points.data(), 9, toUCol32(color), ImDrawFlags_None, 2);
		};
		doDraw(float4(0, 0, 0, 0.5), 4);
		doDraw(float4(color, 1), 2);
	}
											 ...
											 float radius, float angle, mtl::float3 color)
	{
		...
		float4x4 const transform = entity.scene().calculateTransformRelativeToWorld(entity);
		...
		std::transform(circlePoints1.begin(), circlePoints1.end(),
					   ...
					   [this](float3 x) { return viewport->worldSpaceToWindowSpace(x).xy; });
	...
	template <typename Light>
	EntityHandle ViewportOverlays::hitTestLightOverlays(mtl::float2 hitPositionInView) {
		for (auto&& scene: viewport->editor().coreSystems().sceneSystem().scenes()) {
			auto componentView = scene->view<TransformMatrixComponent const, Light const>();
			for (auto&& [id, transform, light]: componentView.each()) {
				auto const entityPositionInView = viewport->worldSpaceToViewSpace(transform.matrix.column(3).xyz);
				...
				bool const result = hitTestOneLightOverlay<Light>(entityPositionInView.xy, hitPositionInView);
				if (result) {
					return entity;
				}
			}
	...
	template <typename Light>
	bool ViewportOverlays::hitTestOneLightOverlay(mtl::float2 entityPositionInWindow,
												  mtl::float2 hitPositionInWindow)
	{
