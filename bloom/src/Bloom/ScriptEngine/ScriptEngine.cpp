...
//		return const_cast<chaiscript::ChaiScript::State&>(asState((void
// const*)ptr));
//	}
//
...
//	}
//
//	void ScriptEngine::registerFunction(chaiscript::Proxy_Function&& fn,
// std::string&& name) { 		asChai(_engine.get()).add(std::move(fn),
// std::move(name));
//	}
//
//	void ScriptEngine::registerType(chaiscript::Type_Info&& type,
// std::string&& name) { 		asChai(_engine.get()).add(std::move(type),
// std::move(name));
//	}
//
//	void ScriptEngine::registerConversion(chaiscript::Type_Conversion&&
// conv) { 		asChai(_engine.get()).add(std::move(conv));
//	}
//
...
// ScriptEngine::instanciateObject(std::string_view className) { 		return
// eval<std::shared_ptr<chaiscript::dispatch::Dynamic_Object>>(utl::format("{}()",
// className));
//	}
//
//	ScriptEngine::State ScriptEngine::getState() {
//		return State(new
// chaiscript::ChaiScript::State(asChai(_engine.get()).get_state()));
//	}
//
...
//		_state(new
// chaiscript::ChaiScript::State(asState(rhs._state.get())))
//	{
//
//	}
//
//	ScriptEngine::State& ScriptEngine::State::operator=(State const& rhs) {
//		_state = std::unique_ptr<void, Deleter>(new
// chaiscript::ChaiScript::State(asState(rhs._state.get()))); 		return
// *this;
//	}
//
