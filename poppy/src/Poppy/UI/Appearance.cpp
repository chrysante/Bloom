...
namespace poppy {
	...
	StyleColors makeStyle(float hue, StyleColorDescription const& desc) {
		StyleColors result{};
		...
		result.highlightControlFrame = mtl::hsva_to_rgba(desc.highlightControlFrame);
		result.highlightControlFrameHovered = mtl::hsva_to_rgba(desc.highlightControlFrameHovered);
		result.highlightControlFrameDown = mtl::hsva_to_rgba(desc.highlightControlFrameDown);
	...
	void Appearance::deserialize(YAML::Node root) {
		try {
			lightColorDesc = root["Light Color Description"].as<StyleColorDescription>();
			darkColorDesc = root["Dark Color Description"].as<StyleColorDescription>();
			mStyle.vars = root["Style Vars"].as<StyleVars>();
		}
		catch (...) {
			poppyLog(error, "Failed to load Appearance");
		}
	...
	void Appearance::showInspector(bool* open) {
		...
		if (ImGui::BeginTabBar("##tabs", ImGuiTabBarFlags_None)) {
			if (ImGui::BeginTabItem("Sizes")) {
				ImGui::Text("Borders");
				update |= ImGui::Checkbox("Window Border", &mStyle.vars.windowBorder);
				update |= ImGui::Checkbox("Child Window Border", &mStyle.vars.childWindowBorder);
				...
				update |= ImGui::SliderFloat("Window Rounding", &mStyle.vars.windowRounding, 0, 12, "%.0f");
				update |= ImGui::SliderFloat("Child Window Rounding", &mStyle.vars.childWindowRounding, 0, 12, "%.0f");
				update |= ImGui::SliderFloat("Popup Rounding", &mStyle.vars.popupRounding, 0, 12, "%.0f");
				update |= ImGui::SliderFloat("Frame Rounding", &mStyle.vars.frameRounding, 0, 12, "%.0f");
				update |= ImGui::SliderFloat("Tab Rounding", &mStyle.vars.tabRounding, 0, 12, "%.0f");
				update |= ImGui::SliderFloat("Selectable Rounding", &mStyle.vars.selectableRounding, 0, 12, "%.0f");
				
				ImGui::Text("Grab");
				update |= ImGui::SliderFloat("Grab Min Size", &mStyle.vars.grabMinSize, 1, 20, "%.0f");
				update |= ImGui::SliderFloat("Grab Rounding", &mStyle.vars.grabRounding, 0, 12, "%.1f");
				
				ImGui::Text("Scrollbar");
				update |= ImGui::SliderFloat("Scrollbar Size", &mStyle.vars.scrollbarSize, 1, 20, "%.0f");
				update |= ImGui::SliderFloat("Scrollbar Rounding", &mStyle.vars.scrollbarRounding, 0, 12, "%.0f");
				
				ImGui::Text("Misc");
				update |= ImGui::SliderFloat2("Window Title Align", &mStyle.vars.windowTitleAlign, 0.0f, 1.0f, "%.2f");
			...
			if (ImGui::BeginTabItem("Colors")) {
				auto item = [&](char const* name, UIColor& color) -> bool {
					bool const result = ImGui::ColorEdit4(name, color.data(),
				...
				auto& desc = mType == AppearanceType::light ? lightColorDesc : darkColorDesc;
				...
				update |= item("Highlight Control Frame", desc.highlightControlFrame);
				update |= item("Highlight Control Frame Hovered", desc.highlightControlFrameHovered);
				update |= item("Highlight Control Frame Down", desc.highlightControlFrameDown);
	...
	static bool colorEdit(float4& color, char const* label) {
		...
		flags |= ImGuiColorEditFlags_NoInputs | ImGuiColorEditFlags_AlphaBar | ImGuiColorEditFlags_Float;
