...
template <>
struct ToInputEvent<InputEventType::leftMouseDown> {
    using type = MouseDownEvent;
};
template <>
struct ToInputEvent<InputEventType::rightMouseDown> {
    using type = MouseDownEvent;
};
template <>
struct ToInputEvent<InputEventType::otherMouseDown> {
    using type = MouseDownEvent;
};
template <>
struct ToInputEvent<InputEventType::mouseDown> {
    using type = MouseDownEvent;
};

template <>
struct ToInputEvent<InputEventType::leftMouseUp> {
    using type = MouseUpEvent;
};
template <>
struct ToInputEvent<InputEventType::rightMouseUp> {
    using type = MouseUpEvent;
};
template <>
struct ToInputEvent<InputEventType::otherMouseUp> {
    using type = MouseUpEvent;
};
template <>
struct ToInputEvent<InputEventType::mouseUp> {
    using type = MouseUpEvent;
};

template <>
struct ToInputEvent<InputEventType::mouseMoved> {
    using type = MouseMoveEvent;
};

template <>
struct ToInputEvent<InputEventType::leftMouseDragged> {
    using type = MouseDragEvent;
};
template <>
struct ToInputEvent<InputEventType::rightMouseDragged> {
    using type = MouseDragEvent;
};
template <>
struct ToInputEvent<InputEventType::otherMouseDragged> {
    using type = MouseDragEvent;
};
template <>
struct ToInputEvent<InputEventType::mouseDragged> {
    using type = MouseDragEvent;
};

template <>
struct ToInputEvent<InputEventType::scrollWheel> {
    using type = ScrollEvent;
};
template <>
struct ToInputEvent<InputEventType::magnify> {
    using type = MagnificationEvent;
};

template <>
struct ToInputEvent<InputEventType::keyDown> {
    using type = KeyEvent;
};
template <>
struct ToInputEvent<InputEventType::keyUp> {
    using type = KeyEvent;
};
template <>
struct ToInputEvent<InputEventType::key> {
    using type = KeyEvent;
};

template <typename E>
struct IsInputEvent:
    std::bool_constant<std::is_base_of_v<InputEventBase, std::decay_t<E>>> {};
} // namespace internal
...
public:
    ...
    auto visit(auto&& f) const { return std::visit(f, _union); }
    ...
    template <InputEventType Type,
              typename E = typename internal::ToInputEvent<Type>::type>
