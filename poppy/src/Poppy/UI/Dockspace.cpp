...
namespace poppy {
	...
	void Dockspace::mainWindow() {
		...
		ImGui::PushStyleVar(ImGuiStyleVar_FramePadding, float2(1, 1) * (float2)style.FramePadding);
	...
	static void toolbarWindow(char const* id, float posX, float2 const size, Block&& block) {
		...
		if (ImGui::IsWindowFocused() && GImGui->WindowsFocusOrder.size() > 1 && ImGui::GetIO().MouseReleased[ImGuiMouseButton_Left]) {
	...
	void Dockspace::displayToolbar() {
		...
		switch (spacing.size()) {
			...
			case 0:
				break;
			case 1:
				if (spacing[0] >= width * 0.33 && spacing[0] <= width * 0.67) /* in central third */ {
					spacing.clear();
				}
				break;
			case 2:
