...
namespace bloom {
											...
											mtl::usize2 size) const
	{
		...
		for (int i = 0; i < BloomFramebuffer::numDSMipLevels; ++i) {
			framebuffer.downsampleMips[i] = device.createSharedTextureView(framebuffer.downsample,
		...
		for (int i = 0; i < BloomFramebuffer::numUSMipLevels; ++i) {
			framebuffer.upsampleMips[i] = device.createSharedTextureView(framebuffer.upsample,
																		 TextureType::texture2D,
																		 PixelFormat::RG11B10Float,
																		 i, 1, 0, 1);
		}
	...
	void BloomRenderer::render(CommandQueue& commandQueue,
		...
		auto const threadGroupHeight = downsamplePipeline.maxTotalThreadsPerThreadgroup / threadGroupWidth;
		...
		ctx.setTexture(framebuffer.downsampleMips[numDSMips - 1], 0); // upsample source
